package levelscripts.barmodel.tutorialsv2
{
     ...
     */
    public class IntroAddLabel extends BaseCustomLevelScript
    {
        ...
        private var m_expressionNodeBuffer:Vector.<ExpressionNode>;
        ...
        private var m_jobToTotalMapping:Object = {
            ninja: {total_a_name: "months spent training", total_a_abbr: "total months", numeric_unit_a: "months", 
                total_b_name: "disguises mastered", total_b_abbr: "total disguises", numeric_unit_b: "disguises"},
            zombie: {total_a_name: "zombies in horde", total_a_abbr: "total zombies", numeric_unit_a: "zombies", 
                total_b_name: "people ran away", total_b_abbr: "total people", numeric_unit_b: "people"},
            basketball: {total_a_name: "rebounds in drill", total_a_abbr: "total rebounds", numeric_unit_a: "rebounds", 
                total_b_name: "passes in play", total_b_abbr: "total passes", numeric_unit_b: "passes"},
            fairy: {total_a_name: "fairies in forest", total_a_abbr: "total fairies", numeric_unit_a: "fairies", 
                total_b_name: "tree checked", total_b_abbr: "total trees", numeric_unit_b: "trees"},
            superhero: {total_a_name: "rads absorbed", total_a_abbr: "total rads", numeric_unit_a: "rads", 
                total_b_name: "people rescued", total_b_abbr: "total people", numeric_unit_b: "people"}
        };
        
        public function IntroAddLabel(gameEngine:IGameEngine, 
                                      ...
                                      isActive:Boolean=true)
        {
            ...
            m_expressionNodeBuffer = new Vector.<ExpressionNode>();
        ...
        override public function visit():int
        {
            if (m_ready)
            {
                if (m_progressControl.getProgressValueEquals("stage", "add_numbers_vertical_model"))
                {
                    var barWholes:Vector.<BarWhole> = m_barModelArea.getBarModelData().barWholes;
                    var correctBoxValuesAdded:Boolean = false;
                    if (barWholes.length == 2)
                    {
                        // Make sure the second total was not one of the bars added
                        var foundFirstValue:Boolean = false;
                        var foundSecondValue:Boolean = false;
                ...
                else if (m_progressControl.getProgressValueEquals("stage", "add_bracket_vertical_model"))
                {
                    ...
                    var verticalBracketAdded:Boolean = false;
                    if (m_barModelArea.getBarModelData().verticalBarLabels.length > 0)
                    {
                        verticalBracketAdded = m_barModelArea.getBarModelData().verticalBarLabels[0].value == m_partATotalValue;
                    }
                ...
                else if (m_progressControl.getProgressValueEquals("hinting", "firstequation"))
                {
                    ...
                    var addedNumberA:Boolean = false;
                    var addedNumberB:Boolean = false;
                    var addedVariable:Boolean = false;
                    ...
                    {
                        m_expressionNodeBuffer.length = 0;
                        ExpressionUtil.getLeafNodes(root, m_expressionNodeBuffer);
                        for each (var leafNode:ExpressionNode in m_expressionNodeBuffer)
                        {
                    ...
                    var equationCorrect:Boolean = addedNumberA && addedNumberB && addedVariable;
        ...
        override protected function onLevelReady():void
        {
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    ...
                    setDocumentIdVisible({id:"multiple_answers_message", visible:true, pageIndex:0});
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    ...
                    var docIdsForNumbers:Vector.<String> = Vector.<String>(["part_a_number_a", "part_a_number_b"]);
                    ...
                    var unitName:String = m_jobToTotalMapping[selectedPlayerJob].numeric_unit_a;
                    ...
                    // Show hints about the vertical model
                    m_progressControl.setProgressValue("stage", "add_numbers_vertical_model");
                    
                    return ScriptStatus.SUCCESS;
                }, null));
            
            sequenceSelector.pushChild(new CustomVisitNode(m_progressControl.getProgressValueEqualsCallback, {key:"stage", value:"add_bracket_vertical_model"}));
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    ...
                    (param.rootNode as BaseCustomLevelScript).pushChild(new HighlightTextProcess(m_textAreaWidget, 
                        Vector.<String>(["part_a_total"]), 0xFF9900, 1, "addunknownlabel"));                    
                    ...
                    setDocumentIdsSelectable(Vector.<String>([docIdsForPartATotal]), true);
                    m_gameEngine.addTermToDocument("total_a", docIdsForPartATotal);
                    
                    return ScriptStatus.SUCCESS;
                }, {rootNode:this}));
            
            sequenceSelector.pushChild(new CustomVisitNode(m_progressControl.getProgressValueEqualsCallback, {key:"stage", value:"parta_finished"}));
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    setDocumentIdVisible({id:"bar_to_equation_instructions", visible:true, pageIndex:1});
                    return ScriptStatus.SUCCESS;
                }, null));
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    var docIds:Vector.<String> = Vector.<String>(["part_b_number_a", "part_b_number_b", "part_b_total"]);
                    setDocumentIdsSelectable(docIds, false, 2);
                    
                    setDocumentIdVisible({id:selectedPlayerJob, visible:true, pageIndex:2});
                    return ScriptStatus.SUCCESS;
                }, null));
            
            sequenceSelector.pushChild(new CustomVisitNode(secondsElapsed, {duration:1.0}));
            sequenceSelector.pushChild(new CustomVisitNode(clickToContinue, {x:470, y:290}));
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    ...
                    // Don't need card restriction
                    getNodeById("RestrictCardsInBarModel").setIsActive(false);
                    
                    var numbersForModel:Vector.<int> = new Vector.<int>();
                    var docIdsForNumbers:Vector.<String> = Vector.<String>(["part_b_number_a", "part_b_number_b"]);
                    var sum:Number = 0;
                    ...
                    // Add unit names to numbers
                    var unitName:String = m_jobToTotalMapping[selectedPlayerJob].numeric_unit_b;
                    var dataForCard:SymbolData = m_gameEngine.getExpressionSymbolResources().getSymbolDataFromValue(m_firstNumberValue);
                    dataForCard.abbreviatedName = m_firstNumberValue + " " + unitName;
                    dataForCard = m_gameEngine.getExpressionSymbolResources().getSymbolDataFromValue(m_secondNumberValue);
                    dataForCard.abbreviatedName = m_secondNumberValue + " " + unitName;
                    ...
                    newBarWhole.barSegments.push(new BarSegment(parseInt(m_firstNumberValue), 1, 0xFFFFFF, null));
                    newBarWhole.barLabels.push(new BarLabel(m_firstNumberValue, 0, 0, false, false, BarLabel.BRACKET_NONE, null));
                    newBarWhole.barSegments.push(new BarSegment(parseInt(m_firstNumberValue), 1, 0xFFFFFF, null));
                    newBarWhole.barLabels.push(new BarLabel(m_secondNumberValue, 1, 1, false, false, BarLabel.BRACKET_NONE, null));
                    newBarWhole.barLabels.push(new BarLabel(m_partBTotalValue, 0, newBarWhole.barSegments.length - 1, true, false, BarLabel.BRACKET_STRAIGHT, null));
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    m_progressControl.setProgressValue("stage", "equation_start_a");
                    return ScriptStatus.SUCCESS;
                }, null));
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    ...
                    m_validateEquation.addEquation("1", 
                        generateSumEquation(Vector.<int>([parseInt(m_firstNumberValue), parseInt(m_secondNumberValue)]), "total_b"), false, true);
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            ...
            {
                if (m_progressControl.getProgressValueEquals("stage", "add_bracket_vertical_model"))
                {
            ...
            {
                if (m_progressControl.getProgressValueEquals("stage", "equation_start_b"))
                {
        ...
        private function generateSumEquation(numbers:Vector.<int>, nameForTotal:String):String
        {
        ...
        private function onSwitchModelClicked(inBarModelMode:Boolean):void
        {
            ...
            else
            {
                ...
                if (m_progressControl.getProgressValueEquals("stage", "equation_start_a"))
                {
                    m_progressControl.setProgressValue("stage", "equation_start_b");
                    m_progressControl.setProgressValue("hinting", "firstequation");
                    removeDialogForUi({id:"switchModelButton"});
                    
                    // Show tooltip on term areas
                    showDialogForUi({id:"barModelArea", text:"Drag the blocks!", color:0x5082B9, direction:Callout.DIRECTION_UP, animationPeriod:1 } );
                }
            }
        }
        ...
        */
        ...
        {
            showDialogForUi({
                id:"barModelArea", text:"Add the numbers on different rows!",
        ...
        {
            showDialogForUi({
                id:"barModelArea", text:"Add the total here!",
                color:0x5082B9, direction:Callout.DIRECTION_UP, width:150, height:50, animationPeriod:1, xOffset:320, yOffset:0
            });
        }
        ...
        {
            showDialogForUi( { id:"leftTermArea", text:"Add both numbers here!", color:0x5082B9, direction:Callout.DIRECTION_UP, animationPeriod:1 } );
        }
        ...
        private function showDragVariableA():void
        {
            ...
            showDialogForUi( { id:uiEntityToAddTo, text:"Put '" + totalName + "' here!", color:0x5082B9, direction:Callout.DIRECTION_UP, animationPeriod:1 } );
        }
