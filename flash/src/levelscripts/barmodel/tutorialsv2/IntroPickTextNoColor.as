package levelscripts.barmodel.tutorialsv2
{
    ...
    import wordproblem.scripts.text.MouseOverImportantText;
    import wordproblem.scripts.text.TextToCard;
    
    /**
     * This is the level to introduce picking parts of the text
     */
    public class IntroPickTextNoColor extends BaseCustomLevelScript
    {
        ...
        */
        ...
        public function IntroPickTextNoColor(gameEngine:IGameEngine, 
                                             ...
                                             isActive:Boolean=true)
        {
            ...
            super.pushChild(new MouseOverImportantText(m_gameEngine, m_expressionCompiler, m_assetManager, onMouseOverTextChanged, "MouseOverImportantText", false));
        ...
        {
            if (m_progressControl.getProgressValueEquals("hinting", "pickgender"))
            {
        ...
        override protected function onLevelReady():void
        {
            ...
            sequenceSelector.pushChild(new CustomVisitNode(
                function(param:Object):int
                {
                    // Activate the mouse over text script that will modify avatar appearance on mouse over
                    (param.rootNode as BaseCustomLevelScript).getNodeById("MouseOverImportantText").setIsActive(true);
            ...
            // Pick a random color and just keep it
            var savedColor:String = m_playerStatsAndSaveData.getPlayerDecision("color") as String;
            if (savedColor == null || savedColor == "start")
            {
                var randomColors:Array = ["red", "orange", "yellow", "green", "blue", "purple"];
                var randomColorIndex:int = Math.floor(Math.random() * randomColors.length);
                m_color = randomColors[randomColorIndex];
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            if (eventType == GameEvent.BAR_MODEL_CORRECT)
            {
                if (m_progressControl.getProgressValueEquals("stage", "pickgender"))
                {
                    ...
                    m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(
                        Vector.<String>(["color_select_b"]),
                        Vector.<XML>([contentB]), 0);
                    m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(
                ...
                {
                    // Deactivate mouse over text since avatar customization is finished
                    getNodeById("MouseOverImportantText").setIsActive(false);
        ...
        private function onMouseOverTextChanged(viewMouseOver:DocumentView, 
                                                viewMousedOut:DocumentView, 
                                                docIdMouseOver:String, 
                                                docIdMouseOut:String):void
        {
            if (m_barModelArea.getBarModelData().barWholes.length == 0)
            {
                if (docIdMouseOver != null)
                {
                    var expressionComponent:ExpressionComponent = m_textAreaWidget.componentManager.getComponentFromEntityIdAndType(
                        docIdMouseOver, ExpressionComponent.TYPE_ID) as ExpressionComponent;
                    if (expressionComponent != null)
                    {
                        if (m_progressControl.getProgressValueEquals("stage", "pickgender"))
                        {
                            m_gender = expressionComponent.expressionString;
                        }
                        else
                        {
                            m_job = expressionComponent.expressionString;
                        }
                    }
                }
                else
                ...
                // If we are mouse over or mouse out of a job, then the avatar should be redrawn
                if (docIdMouseOut != null || docIdMouseOver != null)
                {
                    refreshFirstPageAvatar();
                }
