package gameconfig.versions.challengedemo
{
     ...
     */
    public class WordProblemGameChallengeDemo extends WordProblemGameBase
    {
        /*
        AB Testing names
        */
        private static const AB_VAR_POL_ITEMS:String = "pol";
        private static const AB_VAR_DONGSHENG_ITEMS:String = "dongsheng";
        private static const AB_VAR_CUSTOM_HINTS:String = "customhints";
        private static const POL_ITEMS_DEFAULT:String = "a";
        private static const DONSHENG_ITEMS_DEFAULT:String = "a";
        ...
        public function WordProblemGameChallengeDemo()
        {
            super();
        ...
        override protected function onStartingResourcesLoaded():void
        {
            ...
            loginScreen.addEventListener(CommandEvent.USER_AUTHENTICATED, function(event:Event, params:Object):void
            {
                if (params != null && params.hasOwnProperty("grade"))
                {
                    var userGrade:int = params["grade"];
        ...
        */
        ...
        private function setUpGameForFirstTime(userGrade:int):void
        {
            ...
            var masteryToNodeNameMap:Object = {};
            if (userGrade <= 4)
            {
                masteryToNodeNameMap["1"] = "addition_full_equation";
                masteryToNodeNameMap["2"] = "multipart_subtraction";
                masteryToNodeNameMap["3"] = "multiply_unknown_group";
            }
            else
            {
                masteryToNodeNameMap["1"] = "addition_unknown_total";
                masteryToNodeNameMap["2"] = "multipart_subtraction";
                masteryToNodeNameMap["3"] = "multiply_unknown_group";
            }
            
            masteryToNodeNameMap["4"] = "multiply_multi_operator";
            masteryToNodeNameMap["5"] = "two_step_groups_mixed";
            masteryToNodeNameMap["6"] = "basic_fractions_c";
            masteryToNodeNameMap["7"] = "advanced_fractions_d";
        ...
        override protected function startLevelFromXmlAndExtraData(id:String, levelXml:XML, extraLevelProgressionData:Object=null):void
        {
            ...
            if (extraLevelProgressionData != null && extraLevelProgressionData.hasOwnProperty("isProblemCreate") && extraLevelProgressionData.isProblemCreate)
            {
                ...
                m_assetManager.loadQueue(function(ratio:Number):void
                {
                    ...
                    if (ratio == 1.0)
                    {
                        ...
                        var params:Vector.<Object> = Vector.<Object>([problemCreateData]);
        ...
        private function onCopilotActivityStart(activityDefinition:Object, details:Object):void
        {
            // For this version we ignore level pack since we are using a baked in progressions
            var gradeString:String = "";
            if (details.hasOwnProperty("grade"))
            {
                gradeString = details["grade"];
            }
            
            // Only do anything if no level is active or we have orders to interrupt
            // It is only at this point do we have all the user data necessary for the game to set up
            // properly (in particular we may get the grade that we need)
        ...
        private var m_itemIdsToRemoveForCondition:Vector.<String>;
        private function preprocessLevels(levelObject:Object):void
        {
            m_itemIdsToRemoveForCondition = new Vector.<String>();
            ...
            // For each condition key, each user will have exactly one condition value
            // for that key.
            // For example: key=experiment a, value can be cond1 or cond2
            var userConditions:Object = {};
            userConditions[AB_VAR_POL_ITEMS] = polItemsValue;
            userConditions[AB_VAR_DONGSHENG_ITEMS] = dongshengItemsValue;
            ...
            var levelEdges:Array = levelObject.edges;
            ...
            for (var i:int = 0; i < numEdges; i++)
            {
                ...
                var edge:Object = levelEdges[i];
                if (edge.hasOwnProperty("conditions"))
                {
                    var edgeConditions:Array = edge.conditions;
                    for (var j:int = 0; j < edgeConditions.length; j++)
                    {
                        var condition:Object =  edgeConditions[j];
                        if (condition.hasOwnProperty("name"))
                        {
                            var nodeNameInCondition:String = condition["name"];
                            if (m_itemIdsToRemoveForCondition.indexOf(nodeNameInCondition) > -1)
                            {
         ...
         */
        private function _preprocessLevels(levelObject:Object, userConditions:Object):Boolean
        {
            var levelObjectAllowedForCondition:Boolean = true;
            if (levelObject.hasOwnProperty("condkey") && levelObject.hasOwnProperty("condvalue"))
            {
                var conditionKey:String = levelObject["condkey"];
                var conditionValue:String = levelObject["condvalue"];
                
                // If the level object for a 
                if (userConditions.hasOwnProperty(conditionKey))
                {
                    var userConditionValue:String = userConditions[conditionKey];
            ...
            if (levelObjectAllowedForCondition && levelObject.hasOwnProperty("children"))
            {
                var tempChildrenBuffer:Array = new Array();
                var children:Array = levelObject.children;
                var numChildren:int = children.length;
                var i:int;
                for (i = 0; i < numChildren; i++)
                {
                    var childElement:Object = children[i];
                    var childAllowedForCondition:Boolean = _preprocessLevels(childElement, userConditions);
                ...
                for each (childElement in tempChildrenBuffer)
                {
         ...
         */
        private function getImportantNodeNamesInSequence(grade:int):Vector.<String>
        {
