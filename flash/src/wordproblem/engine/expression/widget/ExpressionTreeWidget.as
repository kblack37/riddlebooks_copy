package wordproblem.engine.expression.widget
{
     ...
     * 
     */ 
    public class ExpressionTreeWidget extends Sprite implements IDisposable
    {
        /**
         ...
         */
        public var showWildCards:Boolean;
        
        public var layoutDivisionVertically:Boolean = true;
         ...
         */
        protected var m_contraintsBox:Rectangle;
        protected var m_vectorSpace:IVectorSpace;
        protected var m_tree:ExpressionTree;
        protected var m_expressionSymbolResources:ExpressionSymbolMap;
         ...
         */
        protected var m_widgetRoot:BaseTermWidget;
         ...
         */
        protected var m_widgetRootGroups:Vector.<BaseTermWidget>;
         ...
         */
        protected var m_nodeIdToWidgetMap:Map;
         ...
         */
        protected var m_allowConstraintPadding:Boolean;
        
        protected var m_assetManager:AssetManager;
        
        protected var m_pointBuffer:Point;
        protected var m_boundsBuffer:Rectangle;
         ...
         */
        public function getWidgetLeaves(outWidgetLeaves:Vector.<BaseTermWidget>):void
        {
        ...
        private function _getWidgetLeaves(widget:BaseTermWidget, 
                                          outWidgetLeaves:Vector.<BaseTermWidget>):void
        {
            if (widget != null)
            {
                                             ...
                                             showWildCards:Boolean=true)
        {
            ...
            m_widgetRootGroups = new Vector.<BaseTermWidget>();
         ...
         */
        public function setConstraints(constraintWidth:Number, 
                                       constraintHeight:Number, 
                                       allowConstraintPadding:Boolean, 
                                       scaleAndLayoutTree:Boolean):void
        {
        ...
        {
            m_scaleFactor = value;
            for (var i:int = 0; i < m_widgetRootGroups.length; i++)
            {
                const widgetRoot:BaseTermWidget = m_widgetRootGroups[i];
        ...
        {
            for each (var widget:BaseTermWidget in m_nodeIdToWidgetMap.getValues())
            {
         ...
         */
        public function refreshNodes(forceCreate:Boolean = false, forceDelete:Boolean = false):void
        {
            // Prune out deleted nodes in our current widget structure  
            const treeRoot:ExpressionNode = m_tree.getRoot();
            var widgetsToDelete:Vector.<BaseTermWidget> = new Vector.<BaseTermWidget>();
            ...
            var i:int;
            for (i = 0; i < widgetsToDelete.length; i++)
            {   
            ...
            var widgetsToCreate:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            _getWidgetsToCreate(treeRoot, m_nodeIdToWidgetMap, widgetsToCreate, forceCreate);
            for (i = 0; i < widgetsToCreate.length; i++)
            {
                ...
                {
                    if (nodeToCreateWidgetFor is WildCardNode)
                    {
                        addedNodeWidget = new WildCardTermWidget(
                            nodeToCreateWidgetFor as WildCardNode,
        ...
        private function _getWidgetsToDelete(widget:BaseTermWidget,
                                             ...
                                             outWidgetToDelete:Vector.<BaseTermWidget>, 
                                             deleteAll:Boolean):void
        {
            if (widget != null)
            {
        ...
        private function _getWidgetsToCreate(node:ExpressionNode,
                                             ...
                                             outNodesToCreate:Vector.<ExpressionNode>, 
                                             createAll:Boolean):void
        {
        ...
        public function getConstraintsWidth():Number
        {
            return this.m_contraintsBox.width;
        }
        
        public function getConstraintsHeight():Number
        {
        ...
        public function getWidgetsMatchingData(data:String, 
                                               ...
                                               outWidgets:Vector.<BaseTermWidget>):void
        {
            if (widgetRoot != null)
            {
        ...
        public function containsObject(displayObject:DisplayObject, 
                                       strictlyContained:Boolean = false):Boolean
        {
         ...
         */
        public function containsPoint(globalPoint:Point):Boolean
        {
        ...
        public function pickLeafWidgetsUnderObject(object:DisplayObject, 
                                                   pickParentheses:Boolean=false, 
                                                   outWidgets:Vector.<BaseTermWidget>=null):Vector.<BaseTermWidget>
        {
            if (outWidgets == null)
            {
                outWidgets = new Vector.<BaseTermWidget>();
            }
        ...
        private function _pickLeafWidgetsUnderObject(widget:BaseTermWidget,
                                                     ...
                                                     outWidgets:Vector.<BaseTermWidget>, 
                                                     pickParentheses:Boolean):void
        {
            if (widget != null)
            {
                const widgetRectangle:Rectangle = widget.rigidBodyComponent.boundingRectangle;
                if (widget is SymbolTermWidget || widget is WildCardTermWidget)
                {
         ...
         */
        public function pickParenthesisUnderPoint(globalX:Number, globalY:Number, outParams:Vector.<Object>):void
        {
                                                    ...
                                                    outParams:Vector.<Object>):void
        {
            ...
            if (widget != null)
            {
                ...
                if (widget.getNode().wrapInParentheses)
                {
                    ...
                    var i:int;
                    for (i = 0; i < numParenthesisChildren; i++)
                    {
                        ...
                        if (pickedParenthesisBounds.containsPoint(localPoint))
                        {
                            // Return a hit
                            pickedWidget = widget;
                            outParams.push(pickedWidget, i == 0);
                            break;
                        }
                    }
                }
         ...
         */
        public function pickWidgetUnderPoint(x:Number, y:Number, allowPickOperator:Boolean):BaseTermWidget
        {
        ...
        private function _pickWidgetUnderPoint(widget:BaseTermWidget, 
                                               localPoint:Point,
                                               allowPickOperator:Boolean):BaseTermWidget
        {
            var pickedWidget:BaseTermWidget;
            if (widget is SymbolTermWidget)
            {
                const widgetBounds:Rectangle = widget.rigidBodyComponent.boundingRectangle;
            ...
            else if (widget != null)
            {
                ...
                if (allowPickOperator && widget is GroupTermWidget)
                {
                    (widget as GroupTermWidget).groupImage.getBounds(this, pickedWidgetRectangle);
                    
                    // Add buffering around the operator so it is easier to pick out,
                    // use a fixed dimension box
                    const fixedDimension:Number = 28;
                    var dx:Number = (pickedWidgetRectangle.width < fixedDimension) ?
                        (fixedDimension - pickedWidgetRectangle.width) * 0.5 : 0;
                    var dy:Number = (pickedWidgetRectangle.height < fixedDimension) ?
        ...
        public function buildTreeWidget():void
        {
            ...
            const rootWidget:BaseTermWidget = _buildTreeWidgetLinks(m_tree.getRoot());
        ...
        private function _buildTreeWidgetLinks(node:ExpressionNode):BaseTermWidget
        {
            ...
            if (node != null)
            {
                ...
                if (!node.hidden)
                {
                    createdWidget = m_nodeIdToWidgetMap.get(node.id) as BaseTermWidget;
                    ...
                    else if (!node.isLeaf())
                    {
                        ...
                        const leftWidget:BaseTermWidget = _buildTreeWidgetLinks(node.left);
                        const rightWidget:BaseTermWidget = _buildTreeWidgetLinks(node.right);
        ...
        public function scaleAndLayoutTreeWidgetGroups(rootWidget:BaseTermWidget):void
        {
            ...
            {
                var scaleAmountToFitWidth:Number = m_contraintsBox.width / rootRectangle.width;
                var scaleAmountToFitHeight:Number = m_contraintsBox.height / rootRectangle.height;
        ...
        private function _buildTermWidgetGroups(rootWidget:BaseTermWidget):void
        {
            ...
            if (rootWidget is GroupTermWidget)
            {
                var groupWidget:GroupTermWidget = rootWidget as GroupTermWidget;
                ...
                if (groupNode.isSpecificOperator(m_vectorSpace.getDivisionOperator()) && this.layoutDivisionVertically)
                {
                    ...
                    var leftCenterX:Number = leftWidget.rigidBodyComponent.getCenterOfMass().x;
                    ...
                    var rightCenterX:Number = rightWidget.rigidBodyComponent.getCenterOfMass().x;
                    ...
                    const divisionImage:DisplayObject = groupWidget.groupImage;
            ...
            if (rootWidget.getNode().wrapInParentheses)
            {
                ...
                var leftOffset:Number = centralPointInLocal.x - currentRectangle.left;
                var topOffset:Number = centralPointInLocal.y - currentRectangle.top;
                var parenPadding:Number = 5;
                
                const leftParen:DisplayObject = new Image(m_assetManager.getTexture("paren_left"));
                ...
                const rightParen:DisplayObject = new Image(m_assetManager.getTexture("paren_right"));
        ...
        private function layoutHorizontally(root:BaseTermWidget, left:BaseTermWidget, right:BaseTermWidget):void
        {
            ...
            var xOffset:Number = root.mainGraphicBounds.width * 0.5;
            
            const leftRightPadding:Number = xOffset * 0.5;
        ...
        private function setNodePositions(widget:BaseTermWidget):void
        {
            if (widget != null)
            {
                ...
                const widgetPosition:Point = this.globalToLocal(widgetGlobalCoordinates);
                const previousPosition:Vector3D = widget.getNode().position;
