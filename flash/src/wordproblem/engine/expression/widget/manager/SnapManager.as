package wordproblem.engine.expression.widget.manager
{
     ...
     */
    public class SnapManager implements IDisposable
    {
        /**
         ...
         */
        private var m_widgetCandidates:Vector.<BaseTermWidget>;
        
        /**
         * List of bounding rectangles for each candidate widget, index is the same as in the
         * widget list.
         */
        private var m_widgetCandidateBounds:Vector.<Rectangle>;
         ...
         */
        private var m_widgetCandidateParenthesisBounds:Vector.<Rectangle>;
         ...
         * at the moment.
         */
        private var m_widgetToSnapStartDrag:Boolean;
         ...
         */
        private var m_snapToLeft:Boolean;
         ...
         */
        private var m_tweenRepeatDuration:Number;
        
        public function SnapManager(termArea:TermAreaWidget, 
                                    ...
                                    expressionResourceMap:ExpressionSymbolMap)
        {
            ...
            m_widgetCandidates = new Vector.<BaseTermWidget>();
            m_widgetCandidateBounds = new Vector.<Rectangle>();
            m_widgetCandidateParenthesisBounds = new Vector.<Rectangle>();
            onTermAreaChanged();
        ...
        private const m_localTermAreaMouseBuffer:Point = new Point();
        public function update(mouseState:MouseState, termWidgetToSnap:BaseTermWidget):void
        {
            ...
            */
            ...
            if (termWidgetToSnap != null && m_termArea.isInteractable)
            {
                ...
                const widgetBoundsHalfWidth:Number = m_widgetToSnapLocalBounds.width * 0.5;
                const widgetBoundsHalfHeight:Number = m_widgetToSnapLocalBounds.height * 0.5;
                ...
                const numLeafWidgets:int = m_widgetCandidates.length;
                for (i = 0; i < numLeafWidgets; i++)
                {
                    widgetCandidate = m_widgetCandidates[i];
                    widgetCandidateBounds = m_widgetCandidateBounds[i];
                    
                    var widgetCandidateCenterX:Number = widgetCandidateBounds.x + widgetCandidateBounds.width * 0.5;
                    var widgetCandidateCenterY:Number = widgetCandidateBounds.y + widgetCandidateBounds.height * 0.5;
                    
                    const termWidgetToSnapCenterX:Number = m_widgetToSnapLocalBounds.x + widgetBoundsHalfWidth;
                    const termWidgetToSnapCenterY:Number = m_widgetToSnapLocalBounds.y + widgetBoundsHalfHeight;
                    ...
                    var doPerformSnap:Boolean = false;
                    if (node.isOperator() && 
                    ...
                    else if (!node.isOperator())
                    {
                        ...
                        var intersectRadius:Number = Math.min(m_widgetToSnapLocalBounds.width, m_widgetToSnapLocalBounds.height) * 0.6 * widgetCandidate.scaleX;
                    ...
                    if (doPerformSnap)
                    {
                        var allowMultiply:Boolean = m_levelRules.allowMultiply;
                        var allowDivide:Boolean = m_levelRules.allowDivide;
                        ...
                        var deltaX:Number = widgetCandidateCenterX - termWidgetToSnapCenterX;
                        var deltaY:Number = widgetCandidateCenterY - termWidgetToSnapCenterY;
                        ...
                        const isInFraction:Boolean = ExpressionUtil.isNodePartOfFraction(m_vectorSpace, node);
                        if (isInFraction)
                        {
                            if (allowMultiply)
                            {
                ...
                m_widgetAcceptingSnap != widgetAcceptingSnap)
            {
                ...
                if (m_widgetAcceptingSnap != null && !(m_widgetAcceptingSnap is WildCardTermWidget))
                {
                ...
                if (widgetAcceptingSnap != null && !(widgetAcceptingSnap is WildCardTermWidget))
                {
                    if (termWidgetToSnap != null) {
        ...
        public function getSnapToLeft():Boolean
        {
            return m_snapToLeft;
        }
        ...
        private function onTermAreaChanged():void
        {
            ...
            m_widgetCandidates.length = 0;
            m_widgetCandidateBounds.length = 0;
            m_widgetCandidateParenthesisBounds.length = 0;
            ...
            var holeIndex:int;
            const numWidgets:int = m_widgetCandidates.length;
            for (i = 1; i < numWidgets; i++)
            {
                ...
                const currentPriorityValue:int = (currentWidget is WildCardTermWidget) ? 1 : 0;
                var prevPriorityValue:int = (prevWidget is WildCardTermWidget) ? 1 : 0;
                while (holeIndex > 0 && currentPriorityValue > prevPriorityValue)
                {
                    ...
                    if (nextHoleIndex > -1)
                    {
                        ...
                        prevPriorityValue = (prevWidget is WildCardTermWidget) ? 1 : 0;
                    }
                }
        ...
        private function _refreshCandidateWidgetList(widget:BaseTermWidget,
                                                     candidateWidgetList:Vector.<BaseTermWidget>, 
                                                     candidateWidgetBoundsList:Vector.<Rectangle>):void
        {
            if (widget != null)
            {
                    ...
                    widget.getNode().wrapInParentheses)
                {
                    ...
                    {
                        m_widgetCandidateParenthesisBounds.push(null, null);
                    }
                }
                                                   ...
                                                   widgetAcceptingSnap:BaseTermWidget):void
        {
            ...
            function onNodeAddedToPreview(event:Event, params:Object):void
            {
                // Timing issue:
                ...
                var operatorImage:DisplayObject = (snappedOperatorCopy as GroupTermWidget).groupImage;
                var scaleXValue:Number = (operator == vectorspace.getDivisionOperator()) ?
