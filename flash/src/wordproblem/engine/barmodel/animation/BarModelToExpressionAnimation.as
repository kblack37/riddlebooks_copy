package wordproblem.engine.barmodel.animation
{
     ...
     */
    public class BarModelToExpressionAnimation implements IAnimatable
    {
        /**
         * TODO: Temp made static so all other scripts needing to manipulate copies of the bar model view
         */
        public static function convertBarModelViewsToSingleImage(barModelViews:Vector.<DisplayObject>,
                                                                 ...
                                                                 outTotalBoundsBuffer:Rectangle=null):Image
        {
            ...
            var barModelViewBounds:Vector.<Rectangle> = new Vector.<Rectangle>();
            ...
            barModelRenderTexture.drawBundled(function():void
            {
                ...
                for (i = 0; i < numViews; i++)
                {
                    ...
                    var xOffset:Number = barModelViewBound.x - totalBoundsBuffer.x;
                    var yOffset:Number = barModelViewBound.y - totalBoundsBuffer.y;
         ...
         * expressionTree:
         */
        private var m_barModelToExpressionMappings:Vector.<Object>;
         ...
         */
        public function BarModelToExpressionAnimation(barModelView:BarModelView,
                                                      ...
                                                      juggler:Juggler)
        {
            m_barModelToExpressionMappings = new Vector.<Object>();
            m_barModelCopyImages = new Vector.<Image>();
            m_subexpressionViews = new Vector.<ExpressionTreeWidget>();
        ...
        {
            m_barModelToExpressionMappings.length = 0;
        ...
        public function dispose():void
        {
            ...
            {
                var mappingObject:Object = m_barModelToExpressionMappings[i];
                var j:int;
                var barModelViews:Vector.<DisplayObject> = mappingObject.barModelViews;
                for (j = 0; j < barModelViews.length; j++)
                {
                    barModelViews[j].alpha = 1.0;
                }
            }
            
            m_barModelCopyImages.length = 0;
            m_subexpressionViews.length = 0;
        }
        
        /**
         * The map function needs to call this in order record the order at which things should be animated
         */
        private function addMapping(barModelViews:Vector.<DisplayObject>, 
                                    subexpression:String, 
                                    expressionView:BaseTermWidget):void
        {
            var mappingObject:Object = {
        ...
        private function createTweensForBarToExpressionData(data:Object):void
        {
            ...
            var barModelViews:Vector.<DisplayObject> = data.barModelViews;
            var totalBoundsBuffer:Rectangle = new Rectangle();
            ...
            var originalSubexpressionScale:Number = subexpressionView.getScaleFactor();
            ...
            if (subexpressionView.getWidgetRoot() is GroupTermWidget)
            {
                var graphicsBounds:Rectangle = (subexpressionView.getWidgetRoot() as GroupTermWidget).mainGraphicBounds;
                subexpressionBoundsForComparison = graphicsBounds.clone();
                targetLocalBounds = (targetTermWidget as GroupTermWidget).mainGraphicBounds;
            }
            else
            {
                subexpressionBoundsForComparison = subexpressionView.getWidgetRoot().rigidBodyComponent.boundingRectangle;
            }
            var scaleAmount:Number = (targetLocalBounds.width * originalSubexpressionScale) / subexpressionBoundsForComparison.width;
            ...
            var deltaX:Number = globalTargetAnchor.x - globalSubexpressionAnchor.x;
            var deltaY:Number = globalTargetAnchor.y - globalSubexpressionAnchor.y;
