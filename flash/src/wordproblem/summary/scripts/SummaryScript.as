package wordproblem.summary.scripts
{
     ...
     */
    public class SummaryScript extends BaseBufferEventScript
    {
        ...
        public var totalScreenWidth:Number = 800;
        public var totalScreenHeight:Number = 600;
        ...
        */
        ...
        public function SummaryScript(gameState:WordProblemGameState,
                                      ...
                                      isActive:Boolean=true)
        {
            ...
            m_rewardsButton.defaultSkin = presentIcon;
            m_rewardsButton.scaleWhenHovering = 1.1;
            ...
            m_helperRenderer = new HelperCharacterRenderSystem(assetManager, Starling.juggler, null);
        }
        ...
        override public function visit():int
        {
            ...
            var timeStep:Number = m_time.currentDeltaSeconds;
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            ...
            if (eventType == GameEvent.LEVEL_COMPLETE)
            {
                ...
                var outNewRewardItemIds:Vector.<String> = new Vector.<String>();
                m_playerItemInventory.outNewRewardItemIds.forEach(function(item:String, index:int, vector:Vector.<String>):void
                {
                    outNewRewardItemIds.push(item);
                });
                
                // Check if any changes happened to an item that has stages
                var outChangedRewardEntityIds:Vector.<String> = new Vector.<String>();
                var outPreviousStages:Vector.<int> = new Vector.<int>();
                var outCurrentStages:Vector.<int> = new Vector.<int>();
                m_playerItemInventory.outChangedRewardEntityIds.forEach(function(item:String, index:int, vector:Vector.<String>):void
                {
                    outChangedRewardEntityIds.push(item);
                    outPreviousStages.push(m_playerItemInventory.outPreviousStages[index]);
                    outCurrentStages.push(m_playerItemInventory.outCurrentStages[index]);
                });
                ...
                var earnedNewStar:Boolean = (levelNode.completionValue == LevelNodeCompletionValues.PLAYED_SUCCESS && currentLevel.statistics.previousCompletionStatus != LevelNodeCompletionValues.PLAYED_SUCCESS);
        ...
        private function applyChangesAtEndOfLevel(doShowRewards:Boolean = false):void
        {
            // Show a basic summary of the goals the player achieved in the last playthrough
            var animationDelay:Number = 0.5;
            m_gameState.getSprite().addChild(m_summaryWidget);
            
            var currentLevel:WordProblemLevelData = m_gameEngine.getCurrentLevel();
            var objectivesToUseInSummary:Vector.<BaseObjective> = new Vector.<BaseObjective>();
            var i:int;
        ...
        {
            var levelUpNotice:ConfirmationWidget = new ConfirmationWidget(800, 600,
                function():DisplayObject
                {
                    var levelUpContainer:Sprite = new Sprite();
                    var coinsForLevelup:int = m_playerCurrencyModel.getCoinsEarnedForLevelUp(String(newLevel));
                    ...
                    var levelUpText:TextField = new TextField(400, measuringText.textHeight + 5, levelUpTextContent, textFormat.font, textFormat.size as int, textFormat.color as uint);
                    ...
                    var coinsEarnedText:TextField = new TextField(100, 40, coinsEarnedTextContent, textFormat.font, 32, textFormat.color as uint);
        ...
        private function onSummaryWidgetButtonPressed():void
        {
            ...
            // HACK: Since the given effect isn't looped, we cannot stop it
            // Forced to have it finish on its own.
            // Audio.instance.stopSfx("win_level");
        }
        
        private function onNextClicked():void
        {
            var loggingDetails:Object = {buttonName:"NextButton"};
        ...
        {
            var loggingDetails:Object = {buttonName:"ExitButton"}            
            m_gameEngine.dispatchEventWith(AlgebraAdventureLoggingConstants.BUTTON_PRESSED_EVENT, false, loggingDetails);
            ...
            {
                m_gameState.dispatchEventWith(CommandEvent.LEVEL_QUIT_AFTER_COMPLETION, false, {level: m_gameEngine.getCurrentLevel()});
            }
            onSummaryWidgetButtonPressed();
        }
