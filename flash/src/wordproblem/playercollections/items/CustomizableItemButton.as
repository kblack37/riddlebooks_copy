package wordproblem.playercollections.items
{
     ...
     */
    public class CustomizableItemButton extends Sprite
    {
        // HACK:
        ...
        private var m_defaultColor:uint;
        private var m_selectedColor:uint;
        
        public function CustomizableItemButton(itemId:String,
                                               categoryId:String,
        ...
        public function setDefaultColor(value:uint):void
        {
            m_defaultColor = value;
        }
        
        public function setSelectedColor(value:uint):void
        {
        ...
        {
            var backgroundColor:uint = m_defaultColor;
            ...
            var playerHasItemAlready:Boolean = m_playerItemInventory.componentManager.getComponentFromEntityIdAndType(m_itemId, ItemIdComponent.TYPE_ID) != null;
            if (!playerHasItemAlready)
            {
                // If player has not purchased the item, the price should show up for it
                if (m_priceContainer == null)
                {
                    var priceComponent:PriceComponent = m_itemDataSource.getComponentFromEntityIdAndType(m_itemId, PriceComponent.TYPE_ID) as PriceComponent;
                    var price:String = priceComponent.price + ""
                    
                    m_priceContainer = new Sprite();
                    
                    var priceIndicatorHeight:Number = m_totalHeight * 0.25;
            ...
            else
            {
                ...
                var equippableComponent:EquippableComponent = m_playerItemInventory.componentManager.getComponentFromEntityIdAndType(m_itemId, EquippableComponent.TYPE_ID) as EquippableComponent;
                if (equippableComponent != null && equippableComponent.isEquipped)
                {
                    if (m_equippedNotice == null)
                    {
                        var equippedTextHeight:Number = m_totalHeight * 0.25;
        ...
        public function createItemIcon():DisplayObject
        {
            ...
            var texturesComponent:TextureCollectionComponent = m_itemDataSource.getComponentFromEntityIdAndType(
                m_itemId, TextureCollectionComponent.TYPE_ID) as TextureCollectionComponent;
            ...
            {
                var buttonColor:uint = parseInt(texturesComponent.textureCollection[0].color, 16);
                ...
                var totalButtonWidth:Number = 60;
