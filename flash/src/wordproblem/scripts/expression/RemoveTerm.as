package wordproblem.scripts.expression
{
     ...
     */
    public class RemoveTerm extends BaseTermAreaScript
    {
        /**
         ...
         */
        private var m_dragRadius:Number = 30;
        
        // Buffer objects
        private var m_mousePoint:Point;
        private var m_selectedWidgetAnchorPoint:Point;
        
        public function RemoveTerm(gameEngine:IGameEngine, 
                ...
                if (m_draggedWidget != null)
                {
                    if (mouseState.leftMouseDraggedThisFrame)
                    {
                        ...
                        if (!MathUtil.pointInCircle(m_selectedWidgetAnchorPoint, m_dragRadius, m_mousePoint))
                        {
                            ...
                            const draggedTermValue:String = m_selectedWidget.getNode().data;
                            var canRemoveTerm:Boolean = true;
                            const nonRemoveableTerms:Vector.<String> = m_levelRules.termsNotRemovable;
                            for (var i:int = 0; i < nonRemoveableTerms.length; i++)
                            {
                                const nonRemoveableValue:String = nonRemoveableTerms[i];
                            ...
                            if (canRemoveTerm)
                            {
                                ...
                                m_gameEngine.dispatchEventWith(GameEvent.REMOVE_TERM, false, {widget:m_selectedWidget});
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            if (eventType == GameEvent.START_DRAG_TERM_AREA)
            {
                ...
                var targetTermArea:TermAreaWidget = param.termArea as TermAreaWidget;
                
                if (selectedWidget is SymbolTermWidget)
                {
         ...
         */
        protected function onRemoveCallback(termArea:TermAreaWidget, termWidget:BaseTermWidget):void
        {
			...
			var loggingDetails_pickup:Object = {expressionName:termWidget.getNode().toString(), regionPickup: uiComponentName, locationX:m_mousePoint.x, locationY:m_mousePoint.y};
