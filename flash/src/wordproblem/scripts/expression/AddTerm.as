package wordproblem.scripts.expression
{
     ...
     */
    public class AddTerm extends BaseTermAreaScript
    {
        private var m_snapManagers:Vector.<SnapManager>;
        ...
        public function AddTerm(gameEngine:IGameEngine, 
        ...
        public function setParams(termAreas:Vector.<TermAreaWidget>,
                                  ...
                                  expressionSymbolMap:ExpressionSymbolMap):void
        {
            ...
            m_snapManagers = new Vector.<SnapManager>();
            m_expressionSymbolMap = expressionSymbolMap;
            
            for (var i:int = 0; i < termAreas.length; i++)
            {
        ...
        override public function visit():int
        {
            if (m_ready && m_isActive)
            {
                ...
                for (var i:int = 0; i < numTermAreas; i++)
                {
                ...
                var showAddPreviewForFrame:Boolean = false;
        ...
        override protected function onLevelReady():void
        {
            ...
            for (var i:int = 0; i < super.m_termAreas.length; i++)
            {
        ...
        private function canPerformAddition():Boolean
        {
            var canPerformAddition:Boolean = true;
            ...
            if (releasedWidgit is SymbolTermWidget)
            {
                ...
                for (var i:int = 0; i < m_termAreas.length; i++)
                {
                        ...
                        (!termArea.restrictValues || termArea.restrictedValues.indexOf(data) > -1))
                    {
                        ...
                        m_eventDispatcher.dispatchEventWith(GameEvent.ADD_TERM_ATTEMPTED, false, {widget:releasedWidgit, success:addSuccessful});
                        if (addSuccessful)
                        {
        ...
        private function attemptToAddSelectedNode(data:String,
                                                  ...
                                                  snapManager:SnapManager):Boolean
        {
            var addSuccessful:Boolean = false;
            ...
            else if (m_levelRules.allowImbalance)
            {
                ...
                if (snapManager.getWidgetAcceptingSnap() != null)
                {
                    ...
                    var attachToLeft:Boolean = snapManager.getSnapToLeft();
                ...
                else if (hitTermArea.isInteractable && canPerformAddition())
                {
                    ...
                    const root:ExpressionNode = (hitTermArea.getWidgetRoot() != null) ? hitTermArea.getWidgetRoot().getNode() : null;
                    
                    if (root != null)
                    {
                        const additiveTermNodes:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                        const nodeGlobalBuffer:Point = new Point();
                        const nodeLocalBuffer:Point = new Point();
                        ExpressionUtil.getAdditiveTerms(root, additiveTermNodes);
                        var additiveTerm:ExpressionNode;
                        for (i = 0; i < additiveTermNodes.length; i++)
                        {
            ...
            else
            {
                ...
                if (snapManager.getWidgetAcceptingSnap() != null )
                {
                    var dataToAdd:Vector.<String> = new Vector.<String>();
                    var operators:Vector.<String> = new Vector.<String>();
                    var dropLocationsX:Vector.<Number> = new Vector.<Number>();
                    var dropLocationsY:Vector.<Number> = new Vector.<Number>();
                    var nodesToAttachTo:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                    var attachToLeftList:Vector.<Boolean> = new Vector.<Boolean>();
                    dataToAdd.push(data);
                    ...
                    const nodeAcceptingSnap:ExpressionNode = snapManager.getWidgetAcceptingSnap().getNode();
                    ...
                    // Need to do this hear since parameters might be cleared in the loop below
                    var groupRoots:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                    ...
                    for (i = 0; i < m_termAreas.length; i++)
                    {
                        ...
                        if (termArea != hitTermArea)
                        {
                            ...
                            dataToAdd = new Vector.<String>();
                            operators = new Vector.<String>();
                            dropLocationsX = new Vector.<Number>();
                            dropLocationsY = new Vector.<Number>();
                            nodesToAttachTo = new Vector.<ExpressionNode>();
                            attachToLeftList = new Vector.<Boolean>();
                            groupRoots = new Vector.<ExpressionNode>();
                            ExpressionUtil.getCommutativeGroupRoots(
        ...
        private function fillGroupsWithWildCards(expectedValue:String,
                                                 groupRoots:Vector.<ExpressionNode>, 
                                                 operator:String,
                                                 nodeToExclude:ExpressionNode,
                                                 outDataToAdd:Vector.<String>,
                                                 outOperators:Vector.<String>,
                                                 outDropLocationsX:Vector.<Number>,
                                                 outDropLocationsY:Vector.<Number>,
                                                 outNodesToAttachTo:Vector.<ExpressionNode>,
                                                 outAttachToLeftList:Vector.<Boolean>,
                                                 termArea:TermAreaWidget):void
        {
            ...
            for each (var groupRoot:ExpressionNode in groupRoots)
            {
                                ...
                                animate:Boolean=true):void
        {
            ...
            // left child
            var isAddedToLeftOfParenthesis:Boolean = false;
            ...
            function onAddedNode(event:Event, data:Object):void
            {
        ...
        public function addNodeBatch(termArea:TermAreaWidget,
                                     ...
                                     dropLocationsX:Vector.<Number>, 
                                     dropLocationsY:Vector.<Number>, 
                                     nodesToAttachTo:Vector.<ExpressionNode>, 
                                     attachToLeft:Vector.<Boolean>):void
        {
            termArea.isReady = false;
            ...
            const numNodesToAdd:int = data.length;
            var i:int;
