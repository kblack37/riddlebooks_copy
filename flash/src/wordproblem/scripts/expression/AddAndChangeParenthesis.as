package wordproblem.scripts.expression
{
     ...
     */
    public class AddAndChangeParenthesis extends BaseTermAreaScript
    {
        /** Color when user has not pressed down on the button*/
        private var m_inactiveColor:uint = 0x6AA2C8;
        /** Color when user has pressed down on the button*/
        private var m_activeColor:uint = 0xF7A028;
         ...
         */
        private var m_fixedYForDraggedParenthesis:Number;
        ...
        private var m_xAnchoredAtLeftEdge:Number;
        private var m_xAnchoredAtRightEdge:Number;
        
        /**
         * List of widgets that a player can stretch or shrink the parenthesis to fit into
         */
        private var m_widgetsParenthesisCanMoveTo:Vector.<BaseTermWidget>;
        ...
        public function AddAndChangeParenthesis(gameEngine:IGameEngine, 
                                                ...
                                                isActive:Boolean=true)
        {
            ...
            m_widgetsParenthesisCanMoveTo = new Vector.<BaseTermWidget>();
            ...
            if (m_ready && m_isActive)
            {
                ...
                if (m_draggedWholeParenthesis.parent != null)
                {
                    ...
                    if (m_mouseState.leftMouseReleasedThisFrame)
                    {
                        ...
                        {
                            var widgetToAddParenthesisTo:BaseTermWidget = m_outParamsBuffer[0] as BaseTermWidget;
                            var termArea:TermAreaWidget = m_outParamsBuffer[1] as TermAreaWidget;
                    ...
                    else 
                    {
                        if (this.getTermWidgetUnderPoint(m_globalPointBuffer, m_outParamsBuffer))
                        {
                            ...
                            widgetToAddParenthesisTo = m_outParamsBuffer[0] as BaseTermWidget;
                            termArea = m_outParamsBuffer[1] as TermAreaWidget;
                ...
                else if (m_widgetWithParenthesisToMove != null)
                {
                    if (m_mouseState.leftMouseReleasedThisFrame)
                    {
                        ...
                        {
                            var baseTermWidget:BaseTermWidget = m_outParamsBuffer[0] as BaseTermWidget;
                    ...
                    else if (m_mouseState.leftMouseDraggedThisFrame)
                    {
                        ...
                        if (this.getMouseWithinAffectedWidgetBounds(m_globalPointBuffer))
                        {
                            if (m_draggedParenImage != null)
                            {
                                ...
                                var localToGlobalYDelta:Number = m_localPointBuffer.y - m_globalPointBuffer.y;
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            if (eventType == GameEvent.PRESS_PARENTHESIS_TERM_AREA)
            {
                ...
                var leftParenSelected:Boolean = param.left;
                ...
                var isInFraction:Boolean = m_termAreaWithParenthesisToMove.layoutDivisionVertically &&
                ...
                m_widgetsParenthesisCanMoveTo.length = 0;
                ...
                var middleY:Number = baseTermWidget.rigidBodyComponent.boundingRectangle.y + baseTermWidget.rigidBodyComponent.boundingRectangle.height * 0.5;
        ...
        override protected function onLevelReady():void
        {
            ...
            commonInit(m_gameEngine.getUiEntity("parenthesisButton") as DisplayObjectContainer);
            this.setIsActive(m_isActive);
        }
        ...
        private function commonInit(containerForParenthesisButton:DisplayObjectContainer):void
        {
            ...
            var parenthesisButtonWidth:Number = 50;
            var parenthesisButtonHeight:Number = 50;
            
            var backgroundTexture:Texture = m_assetManager.getTexture("card_background_square");
            var cornerPadding:Number = 8;
            ...
            var parenScaleFactor:Number = 0.7;
         ...
         *      up or down
         */
        private function getMouseWithinAffectedWidgetBounds(globalPoint:Point):Boolean
        {
            var withinBounds:Boolean = false;
         ...
         */
        private function getTermWidgetClosestToHorizontal(globalPoint:Point, movingLeftParen:Boolean, outParams:Vector.<Object>):Boolean
        {
            // Check whether we are moving the left or the right paren, as this determines the side of the bounds
            // we are comparing
            var foundWidget:Boolean = false;
            ...
            if (movingLeftParen && m_localPointBuffer.x <= m_xAnchoredAtRightEdge)
            {
                ...
                for (i = 0; i < numWidgetsCanMoveParenthesisTo; i++)
                {
            ...
            else if (!movingLeftParen && m_localPointBuffer.x > m_xAnchoredAtLeftEdge)
            {
                ...
                for (i = numWidgetsCanMoveParenthesisTo - 1; i >= 0 ; i--)
                {
                    termWidget = m_widgetsParenthesisCanMoveTo[i];
                    termWidgetBounds = termWidget.rigidBodyComponent.boundingRectangle;
         ...
         */
        private function getTermWidgetUnderPoint(globalPoint:Point, outParams:Vector.<Object>):Boolean
        {
            ...
            var termWidgetUnderPoint:Boolean = false;
        ...
        private function getWidgetsParenthesisCanMoveTo(widget:BaseTermWidget, termArea:TermAreaWidget, outWidgets:Vector.<BaseTermWidget>):void
        {
            if (widget != null)
            {
