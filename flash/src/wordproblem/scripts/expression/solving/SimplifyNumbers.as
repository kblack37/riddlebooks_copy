package wordproblem.scripts.expression.solving
{
     ...
     */
    public class SimplifyNumbers extends BaseTermAreaScript
    {
        private var m_globalPoint:Point;
        
        public function SimplifyNumbers(gameEngine:IGameEngine, 
        ...
        override public function visit():int
        {
            ...
            if (m_ready && m_isActive)
            {
                ...
                if (m_eventTypeBuffer.length > 0)
                {
                    ...
                    {
                        var params:Object = m_eventParamBuffer[indexOfDragRelease];
                        ...
                        for (i = 0; i < m_termAreas.length; i++)
                        {
                            ...
                            if (termArea.containsObject(widget))
                            {
                                if (originTermArea == termArea)
                                {
                                    ...
                                    if (hitWidget != null)
                                    {
                                        var canSimplify:Boolean = termArea.getTree().simplifyNodes(widget.getNode(), hitWidget.getNode());
                                        if (canSimplify)
                                        {
