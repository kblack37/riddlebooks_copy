package wordproblem.scripts.deck
{
     ...
     */
    public class EnterNewCard extends BaseGameScript
    {
        /**
         ...
         */
        private var m_useKeyboard:Boolean;
         ...
         */
        private var m_addedCardValues:Vector.<String>;
         ...
         */
        public function EnterNewCard(gameEngine:IGameEngine, 
                                     ...
                                     isActive:Boolean=true)
        {
            ...
            m_addedCardValues = new Vector.<String>();
        ...
        override public function visit():int
        {
            ...
            if (m_ready && m_isActive && !Layer.getDisplayObjectIsInInactiveLayer(m_deckWidget))
            {
                ...
                {
                    var baseTermWidget:BaseTermWidget = m_deckWidget.getObjectUnderPoint(mouseState.mousePositionThisFrame.x, mouseState.mousePositionThisFrame.y) as BaseTermWidget;
                    if (baseTermWidget != null)
                    {
                        // Check if we pressed on either the button to add a new card
                        // OR a card that was added
                        if (baseTermWidget.getNode().data == "NEW" || m_addedCardValues.indexOf(baseTermWidget.getNode().data) >= 0)
                        {
        ...
        override protected function onLevelReady():void
        {
            ...
            var deckExpressionsToSetTo:Vector.<String> = new Vector.<String>();
            var isHiddenToSetTo:Vector.<Boolean> = new Vector.<Boolean>();
            var renderComponents:Vector.<DisplayObject> = m_deckWidget.getObjects();
            ...
            for (i = 0; i < numRenderComponents; i++)
            {
                ...
                var baseTermWidget:BaseTermWidget = renderComponents[i] as BaseTermWidget;
        ...
        {
            var doAddNewValue:Boolean = true;
            ...
            var currentValuesInDeck:Vector.<String> = new Vector.<String>();
            var currentHiddenValuesInDeck:Vector.<Boolean> = new Vector.<Boolean>();
            var renderComponents:Vector.<DisplayObject> = m_deckWidget.getObjects();
            ...
            for (i = 0; i < numComponents; i++)
            {
                ...
                if (expressionValue != "NEW")
                {
                        ...
                        m_existingValueToEdit == expressionValue)
                    {
                        ...
                        m_addedCardValues[m_addedCardValues.indexOf(expressionValue)] = newData;
                    }
                    else
                    {
