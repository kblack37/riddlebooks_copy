package wordproblem.scripts.barmodel
{
     ...
     */
    public class HoldToCopy extends BaseBarModelScript
    {
        /**
         * A bit of a hack:
        ...
        private var m_fillInProgress:Boolean;
        private var m_transformInProgress:Boolean;
         ...
         */
        private var m_holdDurationMs:Number = 1000;
        
        private var m_timeMsElapsedSincePress:Number;
        
        /**
         * Rate of fill calculated from the hold duration
         */
        private var m_radiansPerMs:Number;
         ...
         */
        private var m_radiansToFill:Number;
         ...
         * In the timer code
         */
        private var m_hasStartedCompletionAnimation:Boolean;
        
        private var m_innerRadius:Number = 16;
        private var m_outerRadius:Number = 32;
        private var m_fillBitmapData:BitmapData;
        
        private var m_outParams:Vector.<Object>;
         ...
         */
        private var m_removeScripts:Vector.<IRemoveBarElement>;
        ...
        public function HoldToCopy(gameEngine:IGameEngine, 
                                   ...
                                   isActive:Boolean=true)
        {
            ...
            m_removeScripts = new Vector.<IRemoveBarElement>();
        }
        ...
        public function addRemoveScript(removeScript:IRemoveBarElement):void
        {
            ...
            if (m_ready)
            {
                (removeScript as BaseGameScript).overrideLevelReady();
            }
        }
        ...
        override public function visit():int
        {
            if (m_ready && m_isActive)
            {
                ...
                if (m_mouseState.leftMousePressedThisFrame)
                {
                    ...
                    {
                        var selectedObject:DisplayObject = m_outParams[0] as DisplayObject;
                        ...
                        if (this.allowCopy)
                        {
                            ...
                            {
                                (m_gameEngine.getUiEntity("middleLayer") as DisplayObjectContainer).addChild(m_holdToCopyDescription);
                            }
                        ...
                        m_hitBarElementId = null;
                        for each (var barWholeView:BarWholeView in barWholeViews)
                        {
                            var barSegmentViews:Vector.<BarSegmentView> = barWholeView.segmentViews;
                            for each (var barSegmentView:BarSegmentView in barSegmentViews)
                            {
                            ...
                            for each (var barLabelView:BarLabelView in barLabelViews)
                            {
                        ...
                        if (m_hitBarElementId == null)
                        {
                            var verticalLabelViews:Vector.<BarLabelView> = m_barModelArea.getVerticalBarLabelViews();
                            for each (var verticalLabelView:BarLabelView in verticalLabelViews)
                            {
                    ...
                    if (!MathUtil.pointInCircle(m_originPoint, 5, m_localBuffer))
                    {
                        ...
                        if (m_removeScripts.length > 0)
                        {
                            m_barToCard.forceTransform(
                                ...
                                this.getNodeById("WidgetDragSystem") as WidgetDragSystem, 
                                m_barModelArea
                            );
                        }
                        
                        // Loop through all the remove scripts and check if the hit element can be deleted.
                        for each (var removeScript:IRemoveBarElement in m_removeScripts)
                        {
                ...
                {
                    var timeDelta:Number = m_time.currentDeltaMilliseconds;
