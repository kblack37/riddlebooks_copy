package wordproblem.scripts.barmodel
{
     ...
     */
    public class RadialMenuControl implements IDisposable
    {
        /**
        ...
        private var m_enabledGestures:Vector.<Boolean>;
         ...
         */
        private var m_radialMenuSegments:Vector.<DisplayObject>;
        private var m_radialMenuSegmentsDisabled:Vector.<DisplayObject>;
        private var m_radialMenuSegmentsOver:Vector.<DisplayObject>;
        ...
        private var m_outerRadius:Number = 60;
        private var m_outerRadiusSq:Number;
        private var m_innerRadius:Number = 30;
        private var m_innerRadiusSq:Number;
         ...
         */
        private var m_separatingAngles:Vector.<Number>;
         ...
         */
        public function RadialMenuControl(mouseState:MouseState, 
                                          ...
                                          disposeSegment:Function=null)
        {
            ...
            m_separatingAngles = new Vector.<Number>();
            m_radialMenuSegments = new Vector.<DisplayObject>();
            m_radialMenuSegmentsOver = new Vector.<DisplayObject>();
            m_radialMenuSegmentsDisabled = new Vector.<DisplayObject>();
        ...
        public function visit():void
        {
            ...
            if (m_radialMenuContainer != null)
            {
                ...
                var mouseRSq:Number = m_localLocationBuffer.x * m_localLocationBuffer.x + m_localLocationBuffer.y * m_localLocationBuffer.y;
                if (mouseRSq >= m_innerRadiusSq && mouseRSq <= m_outerRadiusSq)
                {
                    ...
                    if (m_separatingAngles.length >= 2)
                    {
                        ...
                        for (i = 0; i < numAngles; i++)
                        {
                            ...
                            // account for
                            var firstAngle:Number = m_separatingAngles[i];
                            var secondAngle:Number = m_separatingAngles[i + 1];
        ...
        // so it looks ok
        public function open(enabledGestures:Vector.<Boolean>,
                             xLocation:Number,
                             yLocation:Number,
                             canvas:DisplayObjectContainer):void
        {
            m_enabledGestures = enabledGestures;
            m_separatingAngles.length = 0;
            m_radialMenuSegments.length = 0;
            ...
            */
            ...
            var radPerSegment:Number = Math.PI * 2 / numOptions;
            var rotationOffset:Number = -Math.PI / 2 - radPerSegment / 2;
            ...
            for (i = 0; i < numOptions; i++)
            {
                ...
                var prevAngle:Number = m_separatingAngles[m_separatingAngles.length - 1];
                
                // Clamp so new angle does not exceed one revolutions
                var newAngle:Number = prevAngle + radPerSegment;
        ...
        public function close():void
        {
            ...
            closeTween.onComplete = function():void
            {
                ...
                function disposeSegments(segments:Vector.<DisplayObject>, mode:String):void
                {
                    for each (var segment:DisplayObject in segments)
                    {
        ...
        private function defaultDraw(optionIndex:int, rotation:Number, arcLength:Number, mode:String):DisplayObject
        {
        ...
        {
            (segment as Image).texture.dispose();
            segment.dispose();
        }
    }
}