package wordproblem.scripts.barmodel
{
     ...
     */
    public class RemoveBarSegment extends BaseBarModelScript implements IRemoveBarElement
    {
        /**
         ...
         */
        public var segmentIdsCannotRemove:Vector.<String>;
        ...
        public function RemoveBarSegment(gameEngine:IGameEngine, 
                                         ...
                                         isActive:Boolean=true)
        {
            ...
            this.segmentIdsCannotRemove = new Vector.<String>();
        ...
        public function removeElement(element:DisplayObject):Boolean
        {
            ...
            if (element is BarSegmentView)
            {
                // Remember the index of the bar and the view
                var hitSegmentView:BarSegmentView = element as BarSegmentView;
                var foundSegment:Boolean = false;
                var barWholes:Vector.<BarWhole> = m_barModelArea.getBarModelData().barWholes;
                for (var i:int = 0; i < barWholes.length; i++)
                {
                    var barSegments:Vector.<BarSegment> = barWholes[i].barSegments;
                ...
                m_eventDispatcher.dispatchEventWith(AlgebraAdventureLoggingConstants.REMOVE_BAR_SEGMENT, false, {barModel:m_barModelArea.getBarModelData().serialize()});
            ...
            if (m_ready && m_isActive && !Layer.getDisplayObjectIsInInactiveLayer(m_barModelArea))
            {
                ...
                if (m_mouseState.leftMousePressedThisFrame)
                {
                    if (checkHitSegment(m_outParamsBuffer))
                    {
                        ...
                        m_hitSegmentView = m_barModelArea.getBarWholeViews()[m_outParamsBuffer[0] as int].segmentViews[m_outParamsBuffer[1] as int];
        ...
        {
            var hitSegment:Boolean = BarModelHitAreaUtil.checkPointInBarSegment(outParams, m_barModelArea, m_localMouseBuffer);
            ...
            if (hitSegment)
            {
                ...
                var hitSegmentId:String = targetBarWhole.barSegments[m_outParamsBuffer[1] as int].id;
         ...
         */
        private function getBarWholeIndexFromSegmentId(outParams:Vector.<Object>, barWholes:Vector.<BarWhole>, barSegmentId:String):void
        {
            ...
            for (i = 0; i < numBarWholes; i++)
            {
                ...
                for (j = 0; j < numBarSegments; j++)
                {
                    ...
                    if (barSegment.id == barSegmentId)
                    {
                        ...
                        outParams.push(targetBarWholeIndex, targetBarSegmentIndex);
                        break;
                    }
                }
            ...
            for (i = 0; i < numBarWholes; i++)
            {
                ...
                if (barComparison != null && barComparison.barWholeIdComparedTo == barWholeIdWithRemovedSegment)
                {
                    ...
                    else if (barComparison.segmentIndexComparedTo >= segmentIndexRemoved)
                    {
                        ...
                        {
                            var totalValueUpToIndex:Number = barWholeWithRemovedSegment.getValue(0, barComparison.segmentIndexComparedTo);
        ...
        private function readjustLabelsFromDeletedIndex(barLabels:Vector.<BarLabel>, segmentIndexRemoved:int):void
        {
