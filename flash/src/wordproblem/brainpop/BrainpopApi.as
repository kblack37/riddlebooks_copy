package wordproblem.brainpop
{
     ...
     */
    public class BrainpopApi
    {
        /**
         * This id is needed so we know a particular user account is coming from brainpop
         */
        public static const BRAINPOP_EXTERNAL_SOURCE_ID:int = 8;

        private static const DOMAIN_SANDBOX:String = "qa.brainpop.com"; // change this is qa.brainpop from s4
        private static const DOMAIN_PRODUCTION:String = "www.brainpop.com";
        private static const DOMAIN_LOCAL:String = "localhost/brainpop/dummy.php?command=checklogin";
        
        private static const REQUEST_CHECK_LOGIN:String = "/api/players/logged-in";
        ...
        private var m_loadedFromBrainpopDomain:Boolean;
        
        private var m_overrideDomainCheck:Boolean = false;
        
        private var m_externalIdAuthenticator:ExternalIdAuthenticator;
        
        public function BrainpopApi(logger:AlgebraAdventureLogger,
                                    ...
                                    starlingStage:starling.display.Stage)
        {
            ...
            if (flashStage.loaderInfo.parameters.hasOwnProperty("parentDomain"))
            {
         ...
         */
        public function startCheckLoginPoll(secondsInterval:Number, successCallback:Function):void
        {
        ...
        {
            var totalWidth:Number = m_starlingStage.stageWidth;
            var totalHeight:Number = m_starlingStage.stageHeight;
            
            var scaleFactor:Number = 1.0;
            var maxDimension:Number = 800;
        ...
        {
            checkLoginStatus(function(isLoggedIn:Boolean, brainpopId:String):void
            {
                // Check if log in was detected since when the login polling was started
                if (isLoggedIn)
                {
                    stopCheckLoginPoll();
                    if (m_loginPollSuccessCallback)
                    {
        ...
        private function onCheckLoginComplete(event:Event):void
        {
            ...
            var returnedData:Object = JSON.parse(m_checkLoginUrlLoader.data);
            var isLoggedIn:Boolean = false;
            var playerType:String = "";
            var brainpopId:String = "";
            if (returnedData.hasOwnProperty("EntryID") && returnedData.hasOwnProperty("player_type"))
            {
                playerType = returnedData["player_type"];
                brainpopId = returnedData["EntryID"];
