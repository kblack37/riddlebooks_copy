package wordproblem.log
{
     ...
     */
    public class AlgebraAdventureLogger extends BaseBufferEventScript
    {        
        /**
         * A debug flag whether the log messages should be traced to the console.
         */
        private static var TRACE_LOG:Boolean = false;
        
        /**
         * The number of milliseconds
         */
        private static const INACTIVE_TIME_THRESHOLD_MS:int = 30000;
         ...
         */
        private var m_inMiddleOfQuest:Boolean;
         ...
         */
        private var m_questStartTime:Number;
        ...
        private var m_lastMouseChangeTime:Number;
        private var m_incrementalIdleTime:Number;
        private var m_totalIdleTime:Number;
        ...
        public function AlgebraAdventureLogger(config:AlgebraAdventureConfig, 
                                               forceUid:String,
        ...
        private function logQuestStart(problemData:WordProblemLevelData):void 
        {
            ...
            var details:Object = (problemData.getScriptRoot() is BaseCustomLevelScript) ?
                (problemData.getScriptRoot() as BaseCustomLevelScript).getQuestStartDetails() : {};
            ...
            {
                for each(var aUser:ICgsUser in m_cgsApi.userManager.userList)
                {
            ...
            if (TRACE_LOG)
            {
                trace("Debug logging: " + problemData.getId() + " hash " + details["goalEquation"]);
            }
        ...
        override public function visit():int
        {
            ...
            if (m_inMiddleOfQuest) 
            {
                ...
                // active action.
                if (m_mouseState.mouseDeltaThisFrame.x != 0 
                    || m_mouseState.mouseDeltaThisFrame.y != 0
                    || m_mouseState.leftMousePressedThisFrame) 
                {
                    m_lastMouseChangeTime = m_time.currentTimeMilliseconds;
                }
        ...
        override protected function processBufferedEvent(eventType:String, param:Object):void
        {
            ...
            else if (eventType == GameEvent.LEVEL_SOLVED)
            {
                ...
                loggingDetails["isComplete"] = true;            
                this.logQuestAction(AlgebraAdventureLoggingConstants.LEVEL_FINISHED_EVENT, loggingDetails);
                
                // TODO: Record info for the challenge here (i.e. mark total time spent)
                // Note that mastery recording is coupled to quest logging
                // Need some isolated call that can be made at the end of a level where the logger can check whether
         ...
         */
        public function getCgsUserProperties(savePlayerDataToServer:Boolean, noSqlSaveKey:String):CgsUserProperties
        {
            ...
            // For anonymous users we can force the same uid to be re-used
            // This is important if an anonymous player switches between games
            ...
            // This is a bit redundant, but in some cases we need to save to local cache instead
            // Changing the previous flag is not sufficient by itself
         ...
         */
        public function totalActiveTime():Number 
        {
         ...
         */
        private function logQuestAction(loggingEventType:String, details:Object):void
        {
            ...
            if (m_inMiddleOfQuest)
            {
                const aid:int = AlgebraAdventureLoggingConstants.getAidForLogEvent(loggingEventType);
        ...
        private function onGameStateEnd():void
        {
            ...
            var loggingDetails:Object = {};
            loggingDetails["isComplete"] = (endType == LevelEndTypes.SOLVED_ON_OWN);
            loggingDetails["endType"] = endType;
            
            if (levelDataStats.levelGraphEdgeIdTaken != null)
            {
                loggingDetails["edgeIdTaken"] = levelDataStats.levelGraphEdgeIdTaken;
            }
            
            if (levelDataStats.masteryIdAchieved > -1)
            {
                loggingDetails["masteryId"] = levelDataStats.masteryIdAchieved;
            }
            
            // Should NOT record the time the player spends looking at the summary screen
            // If a level is successfully solved we want to immediately log the quest end
            var totalActiveTime:Number = m_time.currentTimeMilliseconds - m_questStartTime - m_totalIdleTime;
            loggingDetails["timeInQuest"] = totalActiveTime.toString();
        ...
        private function onGameAction(event:Event, params:Object=null):void
        {
