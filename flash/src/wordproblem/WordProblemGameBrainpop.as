package wordproblem
{
     ...
     */
    public class WordProblemGameBrainpop extends WordProblemGameBase
    {
        private static const AB_VAR_GROUP:String = "group";
        private static const AB_VAR_OFFSET:String = "offset";
        ...
        public function WordProblemGameBrainpop()
        {
            super();
        }
        ...
        override protected function onUserAuthenticated():void
        {
            ...
            if (!m_setUpClassAlready)
            {
                ...
                const itemDataSource:ItemDataSource = new ItemDataSource(rawItemData["items"]);
        ...
        private function onBrainpopCheckLogin(isLoggedIn:Boolean, brainpopId:String):void
        {
        ...
        private function onGetUserIdFromExternIdAndSource(response:ResponseStatus, uid:String):void
        {
            ...
            if (uid != null)
            {
                ...
                if (m_stateMachine.getCurrentState() is ExternalLoginTitleScreenState)
                {
         ...
         */
        private function preprocessLevel(levelObject:Object):void
        {
            ...
            var genreToTutorialLevelList:Object = {};
            var levelChildren:Array = levelObject.children;
            var i:int;
            ...
            for (i = 0; i < numChildren; i++)
            {
                ...
                if (childObject.type == "GenreLevelPack")
                {
                    ...
                    {
                        var tutorialListForGenre:Array = genreToTutorialLevelList[genreName];
                        
                        var chapters:Array = childObject.children;
                        if (chapters.length > 0)
                        {
                            // The 'sliding' window of levels in the first set just contains 10 levels
                            var chapterToModify:Object = chapters[0];
                            var levels:Array = chapterToModify.children;
                            var numLevels:int = levels.length;
                            
                            var newLevelSet:Array = new Array();
                            var numLevelsToAddInNewSet:int = 10;
                            for (var j:int = 0; j < numLevelsToAddInNewSet; j++)
                            {
                                ...
                                var levelToAdd:Object = levels[indexToCheck];
                                ...
                                if (levelToAdd.hasOwnProperty("tags"))
                                {
                                    // Each action maps to a tutorial, if that tutorial has not already been added
                                    // to the sequence then we need to add it here.
                                    var levelTags:Array = levelToAdd.tags;
                                    for each(var tag:String in levelTags)
                                    {
                                        for (var k:int = 0; k < tutorialListForGenre.length; k++)
                                        {
                                            var availableTutorial:Object = tutorialListForGenre[k];
                                            var tags:Array = availableTutorial.tags;
        ...
        private function onUpdateGradeAndGender(event:Event, params:Object):void
        {
