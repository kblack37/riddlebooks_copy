package wordproblem.levelselect.scripts
{
     ...
     */
    public class LevelSelectCharacterSequence extends ScriptNode
    {
        ...
        */
        ...
        public function LevelSelectCharacterSequence(assetManager:AssetManager,
                                                     ...
                                                     isActive:Boolean=true)
        {
            ...
            */
            ...
            mainSequence.pushChild(new CustomVisitNode(m_characterController.setCharacterVisible, {id:"Cookie", visible:false}));
            
            // Character tells player to start at green shelf if they are brand new
            m_characterController.moveCharacterTo({id:"Taco", x:880, y:50, velocity:-1});
            if (levelManager.currentLevelProgression.numLevelLeafsPlayed == 0)
            {
                mainSequence.pushChild(new CustomVisitNode(m_characterController.rotateCharacterTo, {id:"Taco", rotation:-Math.PI*0.5, velocity:-1}));
                mainSequence.pushChild(new CustomVisitNode(m_characterController.isStillMoving, {id:"Taco"}));
                mainSequence.pushChild(new CustomVisitNode(m_characterController.rotateCharacterTo, {id:"Taco", rotation:0.3, velocity:1.4}));
                mainSequence.pushChild(new CustomVisitNode(m_characterController.moveCharacterTo, {id:"Taco", x:530, y:335, velocity:500}));
                mainSequence.pushChild(new CustomVisitNode(m_characterController.isStillMoving, {id:"Taco"}));
                mainSequence.pushChild(new CustomVisitNode(m_characterController.showDialogForCharacter, {
                    id:"Taco", text:"Start playing here!", color:0xFFFFFF, direction:Callout.DIRECTION_RIGHT, width:170, padding:0
                }));
                
                // Once played at least one level can get rid of the character
                mainSequence.pushChild(new CustomVisitNode(getNumLevelsPlayedExceedsLimit, {limit:0}));
            }
            mainSequence.pushChild(new CustomVisitNode(m_characterController.setCharacterVisible, {id:"Taco", visible:false}));
            ...
            function getNumLevelsPlayedExceedsLimit(param:Object):int
            {
        ...
        override public function setIsActive(value:Boolean):void
        {
            ...
            else
            {
                ...
                var textureAtlasComponents:Vector.<Component> = m_characterComponentManager.getComponentListForType(RenderableComponent.TYPE_ID);
                var textureAtlasComponent:RenderableComponent;
                const components:int = textureAtlasComponents.length;
                var i:int;
                for (i = 0; i < components; i++)
                {
                    textureAtlasComponent = textureAtlasComponents[i] as RenderableComponent;
