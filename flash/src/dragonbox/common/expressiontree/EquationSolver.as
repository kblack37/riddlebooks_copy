package dragonbox.common.expressiontree
{
    ...
    public class EquationSolver
    {
        ...
        public function solveForKey(root:ExpressionNode, variableToSolveFor:String):ExpressionNode
        {
            ...
            const TRACE_STEPS:Boolean = false;
            if (TRACE_STEPS) { trace("Start:" + rootCopy.toString()); }
            ...
            if (TRACE_STEPS) { trace("After subtract b terms:" + rootCopy.toString()); }
            var assocMoves:int = reduceKeyAssociativeTerms(rootCopy);
            if (TRACE_STEPS) { trace("Key associative Terms reduced:" + rootCopy.toString()); }
            var reduceMoves:int = reduceYOverYTerms(rootCopy);
            if (TRACE_STEPS) { trace("Y/Y Terms reduced:" + rootCopy.toString()); }
            subtractNonKeyMoves = subtractNonKeyTermsFromBothSides(rootCopy);
            if (TRACE_STEPS) { trace("After subtract b terms:" + rootCopy.toString()); }
            var cancelMoves:int = cancelYMinusYTerms(rootCopy);
            if (TRACE_STEPS) { trace("Y-Y canceled:" + rootCopy.toString()); }
            var removeZeroesMoves:int = removeZeroes(rootCopy);
            if (TRACE_STEPS) { trace("0's removed:" + rootCopy.toString()); }
            var keyInNumer:Boolean = areKeyTermsInNumerator(rootCopy);
            var keyInDenom:Boolean = areKeyTermsInDenominator(rootCopy);
            ...
            if (keyInNumer && keyInDenom) {
                ...
                if (TRACE_STEPS) { trace("After mult by key:" + rootCopy.toString()); }
                
            }
            var keyOnLeftSide:Boolean = containsKey(rootCopy.left);
            var keyOnRightSide:Boolean = containsKey(rootCopy.right);
            ...
            if (TRACE_STEPS) { trace("After subtract b terms (2):" + rootCopy.toString()); }
            var subtractKeyMoves:int = 0;
            if (keyOnLeftSide && keyOnRightSide) {
                subtractKeyMoves = subtractKeyTermsFromBothSides(rootCopy);
                if (TRACE_STEPS) { trace("After subtract c terms:" + rootCopy.toString()); }
            }
            var cancelTwoMoves:int = cancelYMinusYTerms(rootCopy);
            if (TRACE_STEPS) { trace("Y-Y canceled (2):" + rootCopy.toString()); }
            var removeTwoMoves:int = removeZeroes(rootCopy);
            if (TRACE_STEPS) { trace("0's removed (2):" + rootCopy.toString()); }
            var divideMoves:int = divideBothSidesByKeyCoefficients(rootCopy);
            if (TRACE_STEPS) { trace("After divide by terms:" + rootCopy.toString()); }
            var finalReduceMoves:int = reduceYOverYTerms(rootCopy);
            if (TRACE_STEPS) { trace("After final reduce Y/Y:" + rootCopy.toString()); }
            var finalCancelMoves:int = cancelYMinusYTerms(rootCopy);
            if (TRACE_STEPS) { trace("Final Y-Y canceled:" + rootCopy.toString()); }
            var finalRemoveZeroesMoves:int = removeZeroes(rootCopy);
            if (TRACE_STEPS) { trace("Final 0's removed:" + rootCopy.toString()); }
            
            if (TRACE_STEPS) { trace("Total number of moves: " + Number(assocMoves + reduceMoves + cancelMoves + removeZeroesMoves + multiplyMoves + subtractNonKeyMoves +subtractNonKeyMoves2
                + subtractKeyMoves + cancelTwoMoves + removeTwoMoves + divideMoves + finalReduceMoves + finalCancelMoves + finalRemoveZeroesMoves))};
        ...
        private function standardizeOnes(root:ExpressionNode):void
        {
            ...
            for each (var leaf:ExpressionNode in leafNodes) {
        ...
        {
            var moves:int = 0;
            var leftTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root.left, leftTerms);
            var rightTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root.right, rightTerms);
            
            // Search thru b1 -> b4 terms by checking pairwise: [b1, b2], [b1, b3], [b1, b4], [b2, b3], [b2, b4], [b3, b4]
            var i:int, j:int, termOne:ExpressionNode, termTwo:ExpressionNode;
            for (i = 0; i < leftTerms.length - 1; i++) {
                termOne = leftTerms[i];
                if (termOne.isOperator() ) { // || termOne.removedFromTree) {
                    continue;
                }
                for (j = i + 1; j < leftTerms.length; j++) {
                    termTwo = leftTerms[j];
            ...
            for (i = 0; i < rightTerms.length - 1; i++) {
                termOne = rightTerms[i];
                if (termOne.isOperator() ) { //|| termOne.removedFromTree) {
                    continue;
                }
                for (j = i + 1; j < rightTerms.length; j++) {
        ...
        public function removeZeroes(root:ExpressionNode):int
        {
            ...
            var additiveTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root, additiveTerms);
            
            for each (var termToCheck:ExpressionNode in additiveTerms) {
                var removeMe:Boolean = false;
                var allLeafNodes:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                ...
                if (allLeafNodes.length > 0) {
                    for each (var leafNode:ExpressionNode in allLeafNodes) {
         ...
         * @return Number of moves needed
         */
        public function reduceYOverYTerms(root:ExpressionNode, keyOnly:Boolean = false):int
        {
            var moves:int = 0;
            var additiveTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root, additiveTerms);
            for each (var additiveTerm:ExpressionNode in additiveTerms) {
                ...
                var numeratorNodes:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                ExpressionUtil.getLeafNodes(additiveTerm.left, numeratorNodes);
                var numTerm:ExpressionNode, denomTerm:ExpressionNode;
                for each (numTerm in numeratorNodes) {
                    ...
                    var denomNodes:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                    ExpressionUtil.getLeafNodes(additiveTerm.right, denomNodes);
                    for each (denomTerm in denomNodes) {
                        if (denomTerm.data == numTerm.data) {
                            ...
                            moves++;
                            break; // only cancel one denom per numerator
                        }
                    }
                }
            }
        ...
        public function reduceKeyAssociativeTerms(root:ExpressionNode):int {
            ...
            var keyOnLeftSide:Boolean = containsKey(root.left);
            ...
            var additiveTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            var additiveTermsWithKey:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            var additiveFactors:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            var keyTerm:ExpressionNode;
            getAdditiveTerms(gatherNode, additiveTerms);
            for (var i:int = 0; i < additiveTerms.length; i++) {
                ...
                if (containsKey(term) && term.data == root.vectorSpace.getMultiplicationOperator()) {
                    ...
                    var termOnLeftSide:Boolean = containsKey(term.left);
                    if (termOnLeftSide) {
        ...
        public function simplifyYtimesOneTerms(root:ExpressionNode):int
        {
            ...
            for each (var additiveTerm:ExpressionNode in additiveTerms) {
                ...
                for (var i:int = 0; i < numeratorNodes.length; i++) {
         ...
         * @return
         */
        public function areKeyTermsInNumerator(root:ExpressionNode):Boolean
        {
            var additiveTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root, additiveTerms);
            var keysInNumerator:Boolean = false;
            for each (var additiveTerm:ExpressionNode in additiveTerms) {
         ...
         * @return
         */
        public function areKeyTermsInDenominator(root:ExpressionNode):Boolean
        {
            var additiveTerms:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(root, additiveTerms);
            var keysInDenominator:Boolean = false;
            for each (var additiveTerm:ExpressionNode in additiveTerms) {
                if (additiveTerm.data == "/") { //ExpressionOperator.DIVIDE) {
        ...
        private function multiplyBothSidesByKey(root:ExpressionNode):int
        {
            ...
            var allKeys:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
        ...
        private function subtractNonKeyTermsFromBothSides(root:ExpressionNode):int
        {
            ...
            var keyOnLeftSide:Boolean = containsKey(root.left);
            var keyOnRightSide:Boolean = containsKey(root.right);
            var gatherNode:ExpressionNode;
            if (keyOnLeftSide) {
                gatherNode = root.left;
            } else {
            ...
            var termsToCheck:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            getAdditiveTerms(gatherNode, termsToCheck);
            var termsToSubtract:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            for each (var termToSubtract:ExpressionNode in termsToCheck) {
                if (!containsKey(termToSubtract)) {
                    termsToSubtract.push(termToSubtract);
                }
            }
            
            for each (var subTerm:ExpressionNode in termsToSubtract) {
                ...
                if (subTerm.isOperator()) {
                    ...
                    applyOperationWithExistingSiblingNode(root.vectorSpace.getAdditionOperator(), termCopy, subtractNode, false); //apply right
                } else {
        ...
        private function subtractKeyTermsFromBothSides(root:ExpressionNode):int
        {
            ...
            for each (var subTerm:ExpressionNode in termsToSubtract) {
                ...
                if (subTerm.isLeaf()) {
                    ...
                    applyOperation(root.vectorSpace.getAdditionOperator(), negativeData, root.left, false); //apply on right side
                }
                else { // the subTerm is an expression like (b*x)
                    var negatedExpression:ExpressionNode = getNegatedExpression(subTerm);
                    applyOperationWithExistingSiblingNode(root.vectorSpace.getAdditionOperator(), negatedExpression, root.left, false); //apply on right side
                }
                ExpressionUtil.removeNode(root, subTerm);
                moves++;                    
            }
        ...
        private function divideBothSidesByKeyCoefficients(root:ExpressionNode):int
        {
            ...
            var moves:int = 0;
            var termsToDivide:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            var termsToMultiply:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
            
            var keyOnLeftSide:Boolean = containsKey(root.left);
            var keyOnRightSide:Boolean = containsKey(root.right);
            if (keyOnLeftSide && keyOnRightSide) {
                throw new Error("ExpressionTreeUtility.divideBothSidesByKeyCoefficient():\n\rUnexpected Key is found on both sides: " + root.toString());
            }
            var gatherNode:ExpressionNode;
            if (keyOnLeftSide) {
                gatherNode = root.left;
            ...
            if (additiveTerms.length != 1) {
                throw new Error("ExpressionTreeUtility.divideBothSidesByKeyCoefficient():\n\rMultiple terms on Key side, expected only coefficients: " + root.toString());
            }
            ...
            if (gatherNode.data == root.vectorSpace.getDivisionOperator()) { 
                ...
                throw new Error("ExpressionTreeUtility.divideBothSidesByKeyCoefficient():\n\rUnexpected operation/node on Key side, expected only coefficients: " + root.toString());
            }
            
            // Perform divides
            for each (var divTerm:ExpressionNode in termsToDivide) {
                if (isKey(divTerm)) {
                    continue;
                }
                // Have to re-fetch additive terms after each divide, the terms will change as a result of the divide
                var termsBeingDivided:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                if (!keyOnLeftSide) { 
                ...
                for each (var termBeingDivided:ExpressionNode in termsBeingDivided) {
                    if (termBeingDivided.data == root.vectorSpace.getDivisionOperator()) { //ExpressionOperator.DIVIDE) {
                        applyOperation(root.vectorSpace.getMultiplicationOperator(), divTerm.data, termBeingDivided.right, false); //apply right
                    } else {
            ...
            // Multiply terms
            for each (var multTerm:ExpressionNode in termsToMultiply) {
                if (isKey(multTerm)) {
                    continue;
                }
                // Have to re-fetch additive terms after each multiply, the terms will change as a result of the divide
                var termsBeingMultiplied:Vector.<ExpressionNode> = new Vector.<ExpressionNode>();
                ...
                for each (var termBeingMultiplied:ExpressionNode in termsBeingMultiplied) {
                    if (termBeingMultiplied.data ==  root.vectorSpace.getDivisionOperator()) { //ExpressionOperator.DIVIDE) {
                        applyOperation(root.vectorSpace.getMultiplicationOperator(), multTerm.data, termBeingMultiplied.left, false); //apply right
                    } else {
        ...
        public function containsKey(root:ExpressionNode):Boolean
        {
         ...
         */
        public function isKey(root:ExpressionNode):Boolean
        {
            if (root.isNegative()) {
         ...
         *         -ABDS, -ABDS
         */
        public function isOppositeSign(v1:String, v2:String): Boolean
        {
        ...
        public function getNegatedExpression(term:ExpressionNode):ExpressionNode
        {
            ...
            else if (term.right.isLeaf() && isKey(term.left)) 
            {
                ...
                throw new Error("ExpressionUtil.solveForKey():\n\rAttempting to negate a non-simple expression: " + term.toString() +"\n\r");
            }
         ...
         */
        public function applyOperation(operation:String, data:String, node:ExpressionNode, onLeftSide:Boolean):ExpressionNode
        {
            ...
            const newSibling:ExpressionNode = new ExpressionNode(node.vectorSpace, data);
         ...
         */
        public function applyOperationWithExistingSiblingNode(operation:String, newSibling:ExpressionNode, node:ExpressionNode, onLeftSide:Boolean):void
        {
            const newOperator:ExpressionNode = new ExpressionNode(node.vectorSpace, operation);
            newOperator.position = new Vector3D(node.position.x, node.position.y);
            if (node.parent != null) {
                if(node.parent.left == node)
                {
         ...
         */
        public function getAdditiveTerms(root:ExpressionNode, outTerms:Vector.<ExpressionNode>):void
        {
            if (root != null) {
         ...
         */
        public function getKeys(node:ExpressionNode, outKeyNodes:Vector.<ExpressionNode>):void
        {
            if (node != null)
            {
                if (isKey(node))
