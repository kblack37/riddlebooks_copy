package dragonbox.common.state
{
	 ...
	 * states.
	 */
	public class StateMachine extends Sprite implements IStateMachine
	{
		private var states:Vector.<IState>;
		private var previousState:Object;
		private var currentState:IState;
		
        /** Temp holder variable to remember what state to go to between animation frames */
		private var stateToTransitionTo:Object;
        
        /** Temp holder variable to remember params to pass between animation frames */
        private var paramsForStateToGoTo:Vector.<Object>;
		...
		private var transitionInProgress:Boolean;
		
		public function StateMachine(stageWidth:Number, stageHeight:Number)
		{
			this.states = new Vector.<IState>();
		}
		...
		public function changeState(state:Object, params:Vector.<Object>=null, transitionFunction:Function=null):void
		{
        ...
        public function getStateInstance(state:Object):IState
        {
            ...
            for (i = 0; i < numStates; i++)
            {
                ...
                if (state == stateToCompare || state is Class && stateToCompare is (state as Class))
                {
		...
		override public function dispose():void
		{
			...
			for each(var state:IState in this.states)
			{
				state.dispose();
			}
		}
        
		private function goToState(state:Object, params:Vector.<Object>):void
		{
            if (this.currentState != null)
            {
                this.removeChild(this.currentState.getSprite());
