 ...
 */
class SimpleAdditionHint extends HintSelectorNode
{
            ...
            equationValidation : ModelSpecificEquation)
    {
        ...
        m_barModelWidget = try cast(m_gameEngine.getUiEntitiesByClass(BarModelAreaWidget)[0], BarModelAreaWidget) catch(e:Dynamic) null;
    ...
    override public function getHint() : HintScript
    {
        ...
        if (HintCommonUtil.getLevelStillNeedsBarModelToSolve(m_gameEngine)) 
        {
            ...
            var boxesToAdd : Array<String> = new Array<String>();
            ...
            var userBoxesAdded : Array<String> = new Array<String>();
            var userTotal : String = null;
            var userBarWholes : Array<BarWhole> = m_barModelWidget.getBarModelData().barWholes;
            if (userBarWholes.length > 0) 
            {
                for (barWhole in userBarWholes)
                {
                    for (barLabel/* AS3HX WARNING could not determine type for var: barLabel exp: EField(EIdent(barWhole),barLabels) type: null */ in barWhole.barLabels)
                    {
                        if (barLabel.bracketStyle == BarLabel.BRACKET_NONE) 
                ...
                for (verticalLabel/* AS3HX WARNING could not determine type for var: verticalLabel exp: EField(ECall(EField(EIdent(m_barModelWidget),getBarModelData),[]),verticalBarLabels) type: null */ in m_barModelWidget.getBarModelData().verticalBarLabels)
                {
                    userTotal = barLabel.value;
                }
            ...
            else 
            {
                ...
                for (expected in boxesToAdd)
                {
                    var expectedFound : Bool = false;
                    for (given in userBoxesAdded)
                    {
                        if (expected == given) 
                        {
                            expectedFound = true;
        ...
        if (hintMessage != null) 
        {
            var hintData : Dynamic = {
                descriptionContent : hintMessage
