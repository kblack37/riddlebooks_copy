...
class SimpleSubtractionHint extends HintSelectorNode
{
    ...
    override public function getHint() : HintScript
    {
        ...
        if (HintCommonUtil.getLevelStillNeedsBarModelToSolve(m_gameEngine)) 
        {
            ...
            else 
            {
                ...
                var addedCorrectComparison : Bool = false;
                var valuesAddedAsBoxes : Array<String> = new Array<String>();
                for (userBarWhole in userBarWholes)
                {
                    ...
                    for (userLabel/* AS3HX WARNING could not determine type for var: userLabel exp: EField(EIdent(userBarWhole),barLabels) type: null */ in userBarWhole.barLabels)
                    {
                ...
                {
                    if (Lambda.indexOf(valuesAddedAsBoxes, difference) >= 0) 
                    {
