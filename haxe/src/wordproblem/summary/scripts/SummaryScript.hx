    ...
    */
    ...
    override private function processBufferedEvent(eventType : String, param : Dynamic) : Void
    {
        ...
        if (eventType == GameEvent.LEVEL_COMPLETE) 
        {
            ...
            m_playerItemInventory.outNewRewardItemIds.forEach(function(item : String, index : Int, vector : Array<String>) : Void
                    {
            ...
            m_playerItemInventory.outChangedRewardEntityIds.forEach(function(item : String, index : Int, vector : Array<String>) : Void
                    {
    ...
    private function applyChangesAtEndOfLevel(doShowRewards : Bool = false) : Void
    {
        ...
        }  // Convert the grade score earned within all objectives to a letter score that can be shown in the summary    // TODO: Having a separate summary widget might be uneccesary  
                            ...
                            m_playerXpBarFillAnimation.start(totalXpBeforeLevel, m_playerXpModel.totalXP, onXpLevelUp, function() : Void
                                    {
                                        ...
                                        }  // added since the start of this just completed level, which assumes coins are only added at the start)    // Animate adding currency (the previous currency amount is taken to be the current minus the amount  
    ...
    private function onXpLevelUp(newLevel : Int) : Void
    {
        ...
        function() : DisplayObject
        {
            ...
            var levelUpText : TextField = new TextField(400, measuringText.textHeight + 5, levelUpTextContent, textFormat.font, Std.parseInt(textFormat.size), try cast(textFormat.color, Int) catch(e:Dynamic) null);
