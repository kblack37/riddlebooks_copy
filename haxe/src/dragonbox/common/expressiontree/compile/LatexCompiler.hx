...
import dragonbox.common.expressiontree.compile.Token;
...
class LatexCompiler implements IExpressionTreeCompiler
{
    ...
    public function setDynamicVariableInformation(dynamicVariableCharacters : Array<String>,
    ...
    public function decompileAtNode(node : ExpressionNode, expandUnits : Bool = true) : String
    {
    ...
    private function runTokenizer(expression : String) : Array<Token>
    {
        ...
        var alphabetUnderscoreRegex : EReg = new EReg('([a-zA-Z_])', "");
        ...
        for (i in 0...expression.length){
            ...
            else if (Lambda.indexOf(m_dynamicVariableCharacters, nextSymbol) != -1) 
            {
            ...
            else 
            {
                ...
                if (currentToken != null) 
                {
                    ...
                    currentToken = null;
                }  // a negative    // this is if the preceding token is an operator or left paren or blank then the minus is    // value or the subtraction operator. The simplest way to determine    // If we come across a '-' it is either a negative modifier for a  
    ...
    private function runShuntingYardAlgorithm(tokens : Array<Token>) : Array<Token>
    {
        ...
        for (i in 0...tokens.length){
            ...
            else if (currentToken.type == Token.RIGHT_PAREN) 
            {
                ...
                while (!foundLeftParen && stack.count > 0)
                {
                    ...
                    var top : Token = try cast(stack.pop(), Token) catch(e:Dynamic) null;
                ...
                if (stack.count > 0) 
                    { }  // we have a paren error    // If stack is empty without finding a left paren  
        ...
        }  // Otherwise push the operator onto the queue    // eror.    // If the token at the top is a paren, we have a mismatched parenthesis    // After the first pass we look at the tokens in the stack  
        ...
        while (stack.count > 0)
        {
            var remainingToken : Token = try cast(stack.pop(), Token) catch(e:Dynamic) null;
