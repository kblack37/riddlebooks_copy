...
class ExpressionUtil 
{
	...
	public static var NUMERIC_REGEX:EReg = ~/(^-?[0-9]+$)/;
	/** Number starts with optional negative sign and only numbers following it */
	public static var VARIABLE_REGEX:EReg = ~/^-?[a-zA-Z]\w*/;
	
	public static function print(node:ExpressionNode, vectorSpace:RealsVectorSpace):String
	{
		var s:String = "";
		if (node != null)
		{
																   ...
																   vectorSpace:RealsVectorSpace):Bool
	{
		...
		{
			var consumedIndices:Map<Int, Bool> = new Map<Int, Bool>();
												 ...
												 expressionB:ExpressionNode):Int
	{
		...
		var expressionDifference:Int = Std.int(Math.abs(expressionCountA - expressionCountB));
		...
		if (expressionA != null && expressionB != null)
		{
			...
			if (expressionDifference != 0)
			{
				...
				distance += Std.int(Math.min(distanceLeft, distanceRight));
			}
			...
			else
			{
				...
				if (expressionsMatch)
				{
					...
					var vectorSpace:RealsVectorSpace = expressionA.vectorSpace;
				...
				if (expressionSetA.length > 0 && expressionSetB.length > 0)
				{
					...
					var expA:ExpressionNode = null;
					var expB:ExpressionNode = null;
	 ...
	 *         fraction node.
	 */
	public static function isNodePartOfFraction(vectorSpace:RealsVectorSpace, 
												node:ExpressionNode):Bool
	{
		var nodeParent:ExpressionNode = null;
	 ...
	 */
	public static function isNodePartOfDenominator(vectorSpace:RealsVectorSpace,
												   node:ExpressionNode):Bool
	{
	...
	public static function wildCardNodeExists(node:ExpressionNode):Bool
	{
		...
		if (node != null)
		{
			...
			{
				// FIXME:
				wildCardExists = Std.instance(node, WildCardNode) != null;// Std.is(node, WildCardNode);// (node is WildCardNode);
			}
		}
											...
											negativesUnique:Bool):Array<String>
	{
		...
		var uniqueSymbols:Map<String, Bool> = new Map<String, Bool>();
		...
		for (leafNode in leafNodes)
		{
			...
			if (symbol != Std.string(vectorSpace.zero()) && symbol.indexOf("?") == -1)
			{
	...
	public static function isNodeNumeric(node:ExpressionNode):Bool
	{
		...
		if (node != null) 
		{
			isNumeric = numericRegex.match(node.data);
		}
		return isNumeric;
	}
	...
	public static function isVariableNode(node:ExpressionNode):Bool
	{
		...
		if (node != null) {
			isVariable = variableRegex.match(node.data);
		}
		return isVariable;
	}
	...
	public static function simplifyNumericLeaves(aNode:ExpressionNode):ExpressionNode
	{
		...
		if (aNode != null && !aNode.isLeaf())
		{
			...
			else 
			{
				...
				if (isNodeNumeric(aNode.right)) 
				{    //simplifications below may now allow simplifications at this level
					if (isNodeNumeric(aNode.left)) 
					{ //the subtrees collapsed to simple numeric nodes, so collapse them again now
						simpleNode = collapseTwoValueNodesIntoOne(aNode.left, aNode.data, aNode.right);
					...
					{ 
						if (!aNode.left.isLeaf())
						{//the left holds a subtree
							if (aNode.left.left != null && isNodeNumeric(aNode.left.left) && (aNode.data == aNode.left.data))
							{ // and there is the same operator at both levels
							...
							if (aNode.left.right != null && isNodeNumeric(aNode.left.right) && (aNode.data == aNode.left.data))
							{ // and there is the same operator at both levels
	...
	public static function simplifyCommonFactors(aNode:ExpressionNode):ExpressionNode
	{
		...
		{        
			if ( (isNodeAVariableFactor(aNode.left) && isNodeAVariableFactor(aNode.right)) && 
				(getVariableFactorName(aNode.left) == getVariableFactorName(aNode.right))) 
			{ //check whether we have factor subtrees with the same variable. e.g. 3*x and 4*x
			...
			{         
				...
				{    
					...
					if (isNodeAVariableFactor(aNode.left.left)) 
					{ //need to peek down another level into subtrees
	...
	{
		var newNode:ExpressionNode = null;
		if (op == leftNode.vectorSpace.getAdditionOperator())
		{
			newNode = new ExpressionNode(leftNode.vectorSpace, Std.string(Std.parseFloat(leftNode.toString()) + Std.parseFloat(rightNode.toString())));
		}
		else if (op == leftNode.vectorSpace.getSubtractionOperator())
		{
			newNode = new ExpressionNode(leftNode.vectorSpace, Std.string(Std.parseFloat(leftNode.toString()) - Std.parseFloat(rightNode.toString())));
		}
		else if (op == leftNode.vectorSpace.getMultiplicationOperator())
		{
			newNode = new ExpressionNode(leftNode.vectorSpace, Std.string(Std.parseFloat(leftNode.toString()) * Std.parseFloat(rightNode.toString())));
		}
		else if (op == leftNode.vectorSpace.getDivisionOperator())
		{
			newNode = new ExpressionNode(leftNode.vectorSpace, Std.string(Std.parseFloat(leftNode.toString()) / Std.parseFloat(rightNode.toString())));
		}
		else
		{
			throw "Illegal operator, or tree case I haven't handled yet";
		}
	 ...
	 */
	public static function compressToSingleTree(nodes:Array<ExpressionNode>, vectorSpace:RealsVectorSpace):ExpressionNode
	{
		...
		var numNonNullNodes:Int = 0;
		var expressionsStack:GenericStack<ExpressionNode> = new GenericStack<ExpressionNode>();
		for (i in 0...numNodes)
		{
			if (nodes[i] != null)
			{
				expressionsStack.add(nodes[i]);
				numNonNullNodes++;
			}
		}
		
		var root:ExpressionNode = null;
		if (numNonNullNodes > 1)
		{
															  ...
															  vectorSpace:RealsVectorSpace):Bool
	{
		...
		var variableToValueMap:Map<String, Float>;
		...
		var replacementValue:Int = 0;
		for (i in 0...iterationsToCheck)
		{
			variableToValueMap = new Map<String, Float>();
			var randomRange:Int = 50;
			for (uniqueExpectedVariable in uniqueExpectedVariables)
			{
				replacementValue = Std.random(randomRange) + 1;
				if (Math.random() > 0.5)
				{
					replacementValue *= -1;
				}
																...
																vectorSpace:RealsVectorSpace):Bool
	{
		...
		var iterationsToCheck:Int = 10;
		var error:Float = 0.0001;
		var variableToValueMap:Map<String, Float> = new Map<String, Float>();
		...
		for (i in 0...iterationsToCheck)
		{
			...
			for (variableToReplace in uniqueVariables)
			{
				...
				var replacementValue:Int = Std.random(randomRange) + 1;
	 ...
	 * in the output list.
	 */
	public static function getUniqueVariables(node:ExpressionNode, vectorSpace:RealsVectorSpace, outVariables:Array<String>):Void
	{
	...
	public static function evaluateWithVariableReplacement(node:ExpressionNode, 
														   variableValueMap:Map<String, Float>, 
														   vectorSpace:RealsVectorSpace):Float
	{
		var value:Float = 0;
		if (node != null) {
			if (node.isOperator())
