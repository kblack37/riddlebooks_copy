...
class WildCardNode extends ExpressionNode
{
    ...
    public static inline var TYPE_TERMINAL_ANY : String = "$TA";
    /** Match any terminal node that is a number */
    public static inline var TYPE_TERMINAL_NUMBER : String = "$TN";
    /** Match any terminal node that is a variable */
    public static inline var TYPE_TERMINAL_VARIABLE : String = "$TV";
    /** Match any subtree node */
    public static inline var TYPE_SUBTREE_ANY : String = "$SA";
    
    /** Matches blank slotted wild card nodes, these are the same as found in normal dragonbox */
    public static inline var TYPE_REGULAR : String = "?";
    ...
    public static function createWildCardNode(vectorSpace : IVectorSpace, data : String) : ExpressionNode
    {
        ...
        var dataParts : Array<Dynamic> = data.split("_", 2);
    ...
    public static function getTypePriority(node : ExpressionNode) : Int
    {
        ...
        {
            var type : String = (try cast(node, WildCardNode) catch(e:Dynamic) null).wildCardType;
            switch (type)
            {
                ...
                case WildCardNode.TYPE_TERMINAL_ANY:
                    priorityValue = 1;
                case WildCardNode.TYPE_TERMINAL_NUMBER, WildCardNode.TYPE_TERMINAL_VARIABLE:
                    priorityValue = 5;
            }
        }
