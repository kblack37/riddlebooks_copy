...
import cgs.logotos.assets.CopilotTos45954V3;
import cgs.logotos.assets.NoUserNameTos40648V1;
import cgs.logotos.assets.SevenToTwelveTos40648V1;
import cgs.logotos.assets.TeacherTos40648V1;
import cgs.logotos.assets.ThirteenOlderTos41035V1;
import cgs.logotos.assets.UserNameTos40648V1;
...
class CGSTos extends Sprite
{
    ...
    public function new(_cgsServerInstance : ICgsServerApi, _callback : Function, _gameName : String, _tos_terms : String, _width : Float = 800, _height : Float = 600, fontName : String = "Vegur")
    {
        ...
        tos_pane_background.graphics.beginGradientFill(GradientType.LINEAR, new Array<Dynamic>(0xF0F0F0, 0xAAAAAA), new Array<Dynamic>(0.8, 0.8), new Array<Dynamic>(0, 255), mat);
        ...
        m_tos_by_clicking.text = ((_tos_terms.localeCompare(COPILOT_45954_V3) == 0)) ? TOS_BY_CLICKING_INSTRUCTIONS_COPILOT : TOS_BY_CLICKING_INSTRUCTIONS;
        ...
        m_tos_by_clicking_background.graphics.beginGradientFill(GradientType.LINEAR, new Array<Dynamic>(0xF0F0F0, 0xAAAAAA), new Array<Dynamic>(0.8, 0.8), new Array<Dynamic>(0, 255), mat);
        ...
        m_disagree_screen_text_background.graphics.beginGradientFill(GradientType.LINEAR, new Array<Dynamic>(0xF0F0F0, 0xAAAAAA), new Array<Dynamic>(0.8, 0.8), new Array<Dynamic>(0, 255), mat);
        ...
        m_under_13_instructions_background.graphics.beginGradientFill(GradientType.LINEAR, new Array<Dynamic>(0xF0F0F0, 0xAAAAAA), new Array<Dynamic>(0.8, 0.8), new Array<Dynamic>(0, 255), mat);
    ...
    private function saveTosStatus(status : Bool) : Void
    {
        if (m_cgsServerInstance != null)
        {
            ...
            var version : Int = 1;
            var tosHash : String = MD5.hash(m_tos_text_shown);
    ...
    private function setupTosTerms(termsType : String) : String
    {
        ...
        if (termsType.localeCompare(NO_USER_NAME_TOS_40648_V1) == 0)
        {
            termsText = new String(new NoUserNameTos40648V1());
        }
        else
        {
            if (termsType.localeCompare(USER_NAME_TOS_40648_V1) == 0)
            {
                termsText = new String(new UserNameTos40648V1());
            }
            else
            {
                if (termsType.localeCompare(THIRTEEN_OLDER_TOS_41035_V1) == 0)
                {
                    m_show_13_under_button = true;
                    termsText = new String(new ThirteenOlderTos41035V1());
                }
                else
                {
                    if (termsType.localeCompare(SEVEN_TO_TWELVE_TOS_41035_V1) == 0)
                    {
                        m_show_13_under_button = true;
                        termsText = new String(new SevenToTwelveTos40648V1());
                    }
                    else
                    {
                        if (termsType.localeCompare(TEACHER_TOS_40648_V1) == 0)
                        {
                            termsText = new String(new TeacherTos40648V1());
                        }
                        else
                        {
                            if (termsType.localeCompare(COPILOT_45954_V3) == 0)
                            {
                                termsText = new String(new CopilotTos45954V3());
                            }
                            else
                            {
