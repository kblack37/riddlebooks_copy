...
class GridFractionModule implements IFractionModule
{
    ...
    {
        return Math.max(Math.ceil((as3hx.Compat.parseFloat(m_fractionSprite.parentView.fraction.numerator)) / (as3hx.Compat.parseFloat(m_fractionSprite.parentView.fraction.denominator))), 1);
    }
    ...
    public function cloneToFractionSprite(cloneFS : FractionSprite) : Void
    {
        ...
        var numGrids : Int = Math.max(Math.ceil((as3hx.Compat.parseFloat(frac.numerator)) / (as3hx.Compat.parseFloat(frac.denominator))), 1);
    ...
    private function fillLogicGrid(logicGrid : Array<Bool>, startIndex : Int, blocksPerSquare : Int, blocksToFill : Int, rowSize : Int, columnSize : Int) : Void
    {
        ...
        if (blocksToFill < blocksPerSquare)
        {
            ...
            {
                workingMaxFilledRows = Math.min(((maxRowsToFill > 0)) ? maxRowsToFill : rowSize, rowSize);
                workingMaxFilledColumns = Math.min(Math.min(((maxColumnsToFill > 0)) ? maxColumnsToFill : columnSize, Math.ceil(blocksToFill / workingMaxFilledRows)), columnSize);
            }
            else
            {
                workingMaxFilledColumns = Math.min(((maxColumnsToFill > 0)) ? maxColumnsToFill : columnSize, columnSize);
                workingMaxFilledRows = Math.min(Math.min(((maxRowsToFill > 0)) ? maxRowsToFill : rowSize, Math.ceil(blocksToFill / workingMaxFilledColumns)), rowSize);
            }
    ...
    public function peelSquaresToSprites(targetSprites : Array<Sprite>, targetBackbones : Array<Sprite> = null) : Void
    {
        ...
        var colors : Array<Dynamic> = VisualizationUtilities.computeColorArray(m_fractionSprite.parentView.foregroundColor, GenConstants.LIGHTEN_FOREGROUND_FACTOR, GenConstants.DARKEN_FOREGROUND_FACTOR);
        var alphas : Array<Dynamic> = [1, 1, 1];
        var ratios : Array<Dynamic> = [GenConstants.INNER_POINT, GenConstants.MIDDLE_POINT, GenConstants.OUTER_POINT];
        ...
        for (gridNum in 0...numTotalUnits)
        {
            ...
            for (i in 0...numColumns)
            {
                for (j in 0...numRows)
                {
                    ...
                    if (logicGrid[currIndex] != null && Lambda.indexOf(indexExclusionList, currIndex) < 0)
                    {
    ...
    {
        // Prepping variables for gradient
        var colors : Array<Dynamic> = VisualizationUtilities.computeColorArray(foregroundColor, GenConstants.LIGHTEN_FOREGROUND_FACTOR, GenConstants.DARKEN_FOREGROUND_FACTOR);
        var alphas : Array<Dynamic> = [1, 1, 1];
        var ratios : Array<Dynamic> = [GenConstants.INNER_POINT, GenConstants.MIDDLE_POINT, GenConstants.OUTER_POINT];
    ...
    /**
		 ...
		 * @param	ratios
		 */
    private function drawSquareSegment(segment : Sprite, numColumns : Int, numRows : Int, logicGrid : Array<Bool>, startIndex : Int, offsetX : Float, offsetY : Float, colors : Array<Dynamic>, alphas : Array<Dynamic>, ratios : Array<Dynamic>, aBackbone : Sprite = null) : Void
    {
        var workingTotalWidth : Float = totalWidth;
        var workingUnitWidth : Float = unitWidth;
        var workingUnitHeight : Float = unitHeight;
    ...
    /**
		 ...
		 * @param	ratios
		 */
    private function drawSegmentBlock(segment : Sprite, blockX : Float, blockY : Float, colors : Array<Dynamic>, alphas : Array<Dynamic>, ratios : Array<Dynamic>) : Void
    {
        var workingTotalWidth : Float = totalWidth;
        var workingUnitWidth : Float = unitWidth;
        var workingUnitHeight : Float = unitHeight;
