...
class AnimationStep
{
    ...
    private function skipCallback() : Void
    {
        ...
        while (m_tweenStartTimes.length > currTweenIndex || m_callbackTimes.length > currCallbackIndex)
        {
            ...
            if (nextTweenStartTime < nextCallbackStartTime)
            {
                ...
                {
                    aTween.target[aProp] = aTween.getValue(aProp);
                }
                currTweenIndex++;
            }
            else
    ...
    private function skipCallback_reverse() : Void
    {
        ...
        while (currTweenIndex >= 0 || currCallbackIndex >= 0)
        {
            ...
            if (nextTweenStartTime >= nextCallbackStartTime)
            {
                ...
                for (aProp in Reflect.fields(tweenValues))
                {
                    aTween.target[aProp] = aTween.getInitValue(aProp);
                }
                currTweenIndex--;
            }
            else
    ...
    {
        while (m_tweens && m_tweens.length > 0)
        {
        ...
        m_tweens = null;
        while (m_tweenStartTimes && m_tweenStartTimes.length > 0)
        {
            m_tweenStartTimes.pop();
        }
        m_tweenStartTimes = null;
        while (m_callbackArgs && m_callbackArgs.length > 0)
        {
            m_callbackArgs.pop();
        }
        m_callbackArgs = null;
        while (m_callbackTimes && m_callbackTimes.length > 0)
        {
            m_callbackTimes.pop();
        }
