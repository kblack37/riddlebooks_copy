...
class CgsLeafCountLock implements ICgsLevelLock
{
    ...
    public function init(lockKeyData : Dynamic = null) : Void
    {
        ...
        m_unlockPackName = (lockKeyData.exists(UNLOCK_PACK_KEY)) ? Reflect.field(lockKeyData, UNLOCK_PACK_KEY) : m_levelManager.currentLevelProgression.nodeName;
        
        // Unlock Status - optional (defaults to DEFAULT_STATUS)
        m_unlockStatus = (lockKeyData.exists(UNLOCK_STATUS_KEY)) ? Reflect.field(lockKeyData, UNLOCK_STATUS_KEY) : DEFAULT_STATUS;
    }
    ...
    public function doesKeyMatch(keyData : Dynamic) : Bool
    {
        ...
        if (keyData != null)
        {
            ...
            // Unlock Pack - optional (defaults to current level progression)
            var keyUnlockPackName : String = (keyData.exists(UNLOCK_PACK_KEY)) ? Reflect.field(keyData, UNLOCK_PACK_KEY) : m_levelManager.currentLevelProgression.nodeName;
            
            // Node status - optional (defaults to played if not defined)
            var keyUnlockStatus : String = (keyData.exists(UNLOCK_STATUS_KEY)) ? Reflect.field(keyData, UNLOCK_STATUS_KEY) : DEFAULT_STATUS;
