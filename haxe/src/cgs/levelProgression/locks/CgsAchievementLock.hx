...
class CgsAchievementLock implements ICgsLevelLock
{
    ...
    public function init(lockKeyData : Dynamic = null) : Void
    {
        if (lockKeyData != null)
        {
            ...
            m_unlockStatus = (lockKeyData.exists(UNLOCK_STATUS_KEY)) ? Reflect.field(lockKeyData, UNLOCK_STATUS_KEY) : true;
        }
    }
    ...
    public function doesKeyMatch(keyData : Dynamic) : Bool
    {
        ...
        else
        {
            ...
            // Defaults to true if not present
            var keyStatus : Bool = (keyData.exists(UNLOCK_STATUS_KEY)) ? Reflect.field(keyData, UNLOCK_STATUS_KEY) : true;
