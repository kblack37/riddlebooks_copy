...
class CgsNodeLock implements ICgsLevelLock
{
    ...
    public function init(lockKeyData : Dynamic = null) : Void
    {
        ...
        m_unlockStatus = (lockKeyData.exists(NODE_STATUS_KEY)) ? Reflect.field(lockKeyData, NODE_STATUS_KEY) : DEFAULT_STATUS;
    }
    ...
    public function doesKeyMatch(keyData : Dynamic) : Bool
    {
        ...
        if (keyData != null)
        {
            ...
            // Node status - optional (defaults if not defined)
            var keyStatus : String = (keyData.exists(NODE_STATUS_KEY)) ? Reflect.field(keyData, NODE_STATUS_KEY) : DEFAULT_STATUS;
