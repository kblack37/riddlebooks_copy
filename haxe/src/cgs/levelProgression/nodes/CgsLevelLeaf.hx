...
class CgsLevelLeaf extends CGSObject implements ICgsLevelLeaf
{
    ...
    private function initFromData(prevLevel : ICgsLevelLeaf, data : Dynamic) : Void
    {
        ...
        m_levelName = (data.exists("name")) ? data.name : null;
        ...
        var lockArray : Array<Dynamic> = (data.exists("locks")) ? data.locks : null;
        if (lockArray != null)
        {
    ...
    public function loadNodeFromCache(userId : String) : Void
    {
        ...
        {
            m_completionValue = user.getSave(nodeSaveName);
        }
        else
        {
    ...
    public function updateNode(nodeLabel : Int, data : Dynamic = null) : Bool
    {
        ...
        if (m_nodeLabel == nodeLabel)
        {
            ...
            for (user/* AS3HX WARNING could not determine type for var: user exp: EField(EIdent(m_userManager),userList) type: null */ in m_userManager.userList)
            {
