...
class CgsLevelPack extends CGSObject implements ICgsLevelPack
{
    ...
    public static inline var NODE_TYPE : String = "CgsLevelPack";
    ...
    public function init(parent : ICgsLevelPack, prevLevel : ICgsLevelLeaf, data : Dynamic = null) : ICgsLevelLeaf
    {
        ...
        if (data.exists("fileName"))
        {
            ...
            try
            {
                var newData : Dynamic = com.adobe.serialization.json.JSON.decode(file);
                if (newData != null)
                {
                    data = newData;
                }
    ...
    private function initFromData(prevLevel : ICgsLevelLeaf, data : Dynamic) : ICgsLevelLeaf
    {
        ...
        m_playSequentially = (data.exists("playSequentially")) ? data.playSequentially : true;
    ...
    private function addSequentialLocks(childObj : Dynamic, previousNode : ICgsLevelNode) : Void
    {
        ...
        if (!childObj.exists("locks"))
        {
            Reflect.setField(childObj, "locks", new Array<Dynamic>());
        }
        var childLocks : Array<Dynamic> = try cast(Reflect.field(childObj, "locks"), Array</*AS3HX WARNING no type*/>) catch(e:Dynamic) null;
    ...
    private function get_numLevelLeafsCompleted() : Int
    {
        ...
        for (childNode in m_levelData)
        {
            ...
            {
                if (Std.is(childNode, ICgsLevelLeaf) && (try cast(childNode, ICgsLevelLeaf) catch(e:Dynamic) null).isComplete)
                {
                    result++;
                }
            }
    ...
    private function get_numLevelLeafsPlayed() : Int
    {
        ...
        for (childNode in m_levelData)
        {
            ...
            {
                if (Std.is(childNode, ICgsLevelLeaf) && (try cast(childNode, ICgsLevelLeaf) catch(e:Dynamic) null).isPlayed)
                {
                    result++;
                }
            }
    ...
    private function get_numLevelLeafsLocked() : Int
    {
        ...
        for (childNode in m_levelData)
        {
            ...
            {
                if (Std.is(childNode, ICgsLevelLeaf) && (try cast(childNode, ICgsLevelLeaf) catch(e:Dynamic) null).isLocked)
                {
                    result++;
                }
            }
    ...
    {
        // Dont forget self
        var result : Float = (isComplete) ? 1 : 0;
        for (childNode in m_levelData)
        {
            if (Std.is(childNode, ICgsLevelPack))
            {
    ...
    {
        // Dont forget self
        var result : Float = (isFullyPlayed) ? 1 : 0;
        for (childNode in m_levelData)
        {
            if (Std.is(childNode, ICgsLevelPack))
            {
    ...
    public function addNodeToProgression(nodeData : Dynamic, parentPackName : String = null, index : Int = -1) : Bool
    {
        ...
        if (nodeName == parentPackName)
        {
            ...
            {
                as3hx.Compat.arraySplice(nodes, index, 0, [node]);
            }
    ...
    public function editNodeInProgression(nameOfNode : String, newNodeData : Dynamic) : Bool
    {
        ...
        for (childNode in nodes)
        {
            ...
            {
                // Recurseif (Std.is(childNode, ICgsLevelPack))
                {
    ...
    public function removeNodeFromProgression(nodeName : String) : Bool
    {
        ...
        for (childNode in nodes)
        {
            ...
            {
                // Recurse until we find the node to be removedif (Std.is(childNode, ICgsLevelPack))
                {
