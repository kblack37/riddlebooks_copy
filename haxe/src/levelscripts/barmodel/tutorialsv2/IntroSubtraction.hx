...
class IntroSubtraction extends BaseCustomLevelScript
{
    ...
    private var m_jobToTotalMapping : Dynamic = {
            ninja : {
                diff_a_name : "more theft than sabatoge",
                diff_a_abbr : "mission difference",
                numeric_unit_larger_a : "theft",
                numeric_unit_smaller_a : "sabatoge",
                diff_b_name : "more scrolls than artifacts",
                diff_b_abbr : "item difference",

            },
            zombie : {
                diff_a_name : "more research than guard",
                diff_a_abbr : "more research",
                numeric_unit_larger_a : "research",
                numeric_unit_smaller_a : "guard",
                diff_b_name : "more banana than flower",
                diff_b_abbr : "parts difference",

            },
            basketball : {
                diff_a_name : "difference in points",
                diff_a_abbr : "points difference",
                numeric_unit_larger_a : "points",
                numeric_unit_smaller_a : "points",
                diff_b_name : "difference in points",
                diff_b_abbr : "points difference",

            },
            fairy : {
                diff_a_name : "more left than stayed",
                diff_a_abbr : "Pygmy difference",
                numeric_unit_larger_a : "left",
                numeric_unit_smaller_a : "stayed",
                diff_b_name : "more stakes than statues",
                diff_b_abbr : "more stakes",

            },
            superhero : {
                diff_a_name : "more feathers than glue",
                diff_a_abbr : "more feathers",
                numeric_unit_larger_a : "feathers",
                numeric_unit_smaller_a : "glue",
                diff_b_name : "more elbows than dropkicks",
                diff_b_abbr : "more elbows",
        ...
        if (m_ready) 
        {
            ...
            if (m_progressControl.getProgressValueEquals("hinting", "parta_add_bars")) 
            {
                ...
                else if (numBarWholesAdded == 2) 
                {
                    ...
                    var removeBarSegment : RemoveBarSegment = try cast(getNodeById("RemoveBarSegment"), RemoveBarSegment) catch(e:Dynamic) null;
                    ...
                    setDocumentIdsSelectable([m_differenceDocId], true, 0);
            ...
            else if (m_progressControl.getProgressValueEquals("hinting", "parta_add_difference")) 
            {
                ...
                else 
                {
                    ...
                    var comparisonAdded : Bool = barWholes[0].barComparison != null || barWholes[1].barComparison != null;
                    if (comparisonAdded) 
                    {
                    ...
                    else 
                    {
                        ...
                        var incorrectlyAdded : Bool = barWholes[0].barSegments.length > 1 || barWholes[1].barSegments.length > 1;
            ...
            else if (m_progressControl.getProgressValueEquals("hinting", "parta_subtraction_operator")) 
            {
                ...
                var usedSubtractionOperator : Bool = leftTermArea.getWidgetRoot() != null && rightTermArea.getWidgetRoot() != null &&
    ...
    override private function onLevelReady() : Void
    {
        ...
        sequenceSelector.pushChild(new CustomVisitNode(
                function(param : Dynamic) : Int
                {
                    ...
                    var firstPartDocIdsForNumbers : Array<String> = ["part_a_number_a", "part_a_number_b"];
                    ...
                    dataForCard.abbreviatedName = largerValue + " " + Reflect.field(m_jobToTotalMapping, selectedPlayerJob).numeric_unit_larger_a;
                    dataForCard = m_gameEngine.getExpressionSymbolResources().getSymbolDataFromValue(smallerValue);
                    dataForCard.abbreviatedName = smallerValue + " " + Reflect.field(m_jobToTotalMapping, selectedPlayerJob).numeric_unit_smaller_a;
                    
                    (try cast(getNodeById("RestrictCardsInBarModel"), RestrictCardsInBarModel) catch(e:Dynamic) null).setTermValuesToIgnore([m_differenceValue]);
    ...
    private function createBarAndEquationModelsFromElements(docIdsForNumbers : Array<String>,
            unknownValue : String,
            unknownDocId : String,
            ...
            pageIndex : Int) : Void
    {
        ...
        for (i in 0...numbersForModel.length){
        ...
        var termValueToBarValue : Dynamic = { };
        Reflect.setField(termValueToBarValue, unknownValue, numbersForModel[largerIndex] - numbersForModel[smallerIndex]);
    ...
    Logic for hints
    */
    private function showAddNumber(textContent : String, yOffset : Float) : Void
    {
    ...
    private function showAddComparison() : Void
    {
        ...
        var xOffset : Float = (boundsLargerBar.width - boundsSmallerBar.width) * 0.5;
    ...
    private function showSubtractionHint() : Void
    {
        ...
        if (rightChildWidget.stage != null) 
        {
            ...
            var operatorDisplay : DisplayObject = (try cast(rightTermArea.getWidgetRoot(), GroupTermWidget) catch(e:Dynamic) null).groupImage;
