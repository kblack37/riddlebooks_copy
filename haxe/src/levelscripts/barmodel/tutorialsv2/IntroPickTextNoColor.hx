 ...
 */
class IntroPickTextNoColor extends BaseCustomLevelScript
{
    ...
    */
    ...
    override private function onLevelReady() : Void
    {
        ...
        sequenceSelector.pushChild(new CustomVisitNode(
                function(param : Dynamic) : Int
                {
                    ...
                    (try cast(param.rootNode, BaseCustomLevelScript) catch(e:Dynamic) null).getNodeById("MouseOverImportantText").setIsActive(true);
        ...
        var savedColor : String = try cast(m_playerStatsAndSaveData.getPlayerDecision("color"), String) catch(e:Dynamic) null;
        if (savedColor == null || savedColor == "start") 
        {
            var randomColors : Array<Dynamic> = ["red", "orange", "yellow", "green", "blue", "purple"];
            ...
            docIdMouseOut : String) : Void
    {
        if (m_barModelArea.getBarModelData().barWholes.length == 0) 
        {
            ...
            {
                var expressionComponent : ExpressionComponent = try cast(m_textAreaWidget.componentManager.getComponentFromEntityIdAndType(
                        docIdMouseOver, ExpressionComponent.TYPE_ID), ExpressionComponent) catch(e:Dynamic) null;
