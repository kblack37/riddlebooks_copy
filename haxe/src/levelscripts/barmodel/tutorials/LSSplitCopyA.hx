...
class LSSplitCopyA extends BaseCustomLevelScript
{
    ...
    private var m_alienOptions : Array<String>;
     ...
     */
    private var m_alienValueToName : Dynamic;
    
    // Hints to get working
    private var m_firstBarModelIsSetup : Bool = false;
    private var m_secondBarModelIsSetup : Bool = false;
            ...
            isActive : Bool = true)
    {
        ...
        m_alienOptions = ["arctria", "boofly", "flarn", "flobi", "goo", "spugsy", "voya"];
        m_alienValueToName = {
                    arctria : "Arctria",
                    boofly : "Boofly",
                    flarn : "Flarn",
    ...
    override public function visit() : Int
    {
        ...
        if (m_ready) 
        {
            ...
            else if (m_progressControl.getProgress() == 3 && m_secondBarModelIsSetup) 
            {
                ...
                var labelOnBottomBar : Bool = false;
                var createdBarFromCopy : Bool = false;
                ...
                if (splitPerformed && !createdBarFromCopy) 
                {
                    ...
                    else if (m_hintController.getCurrentlyShownHint() != m_addNewBoxHint && draggedWidget != null && !(Std.is(draggedWidget, SymbolTermWidget))) 
                    {
    ...
    override private function processBufferedEvent(eventType : String, param : Dynamic) : Void
    {
        if (eventType == GameEvent.BAR_MODEL_CORRECT) 
        {
            if (m_progressControl.getProgress() == 0) 
            {
                ...
                contentA.node.appendChild.innerData(Reflect.field(m_alienValueToName, m_selectedAlienA));
                m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(["alien_a_a"],
                        [contentA], 0);
                
                // Replace words on other pages
                contentA = FastXML.parse("<span></span>");
                contentA.node.appendChild.innerData(Reflect.field(m_alienValueToName, m_selectedAlienA));
                m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(["alien_a_b", "alien_a_c"],
                        [contentA, contentA], 1);
                
                m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(["alien_a_d"],
                        [contentA], 3);
            ...
            else if (m_progressControl.getProgress() == 2) 
            {
                ...
                // Replace words on other pages
                contentA = FastXML.parse("<span></span>");
                contentA.appendChild(Reflect.field(m_alienValueToName, m_selectedAlienB));
                m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(["alien_b_a"],
                        [contentA], 2);
                
                // Replace words on other pages
                contentA = FastXML.parse("<span></span>");
                contentA.appendChild(Reflect.field(m_alienValueToName, m_selectedAlienB));
                m_textReplacementControl.replaceContentAtDocumentIdsAtPageIndex(["alien_b_b"],
                        [contentA], 3);
    ...
    private function setupPickSecondAlienModel() : Void
    {
        ...
        var prunedAlienOptions : Array<String> = new Array<String>();
        for (alienOption in m_alienOptions)
        {
    ...
    private function setupUseSplitAndCopyTool() : Void
    {
        ...
        var deck : Array<String> = ["5", m_selectedAlienB];
        m_gameEngine.setDeckAreaContent(deck, super.getBooleanList(deck.length, false), false);
    ...
    */
    private function showFirstSplit() : Void
    {
        ...
        showDialogForBaseWidget({
                    id : "2",
                    widgetId : "deckArea",
                    text : "Drag onto box to divide into 2",
                    color : 0xFFFFFF,
    ...
    private function showSecondSplit() : Void
    {
        ...
        showDialogForBaseWidget({
                    id : "5",
                    widgetId : "deckArea",
                    text : "Drag onto box to divide into 5",
                    color : 0xFFFFFF,
                    direction : Callout.DIRECTION_RIGHT,
                    width : 200,
                    height : 70,
